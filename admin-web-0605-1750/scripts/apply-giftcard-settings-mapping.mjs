/**
 * 将礼品卡中心 2 组分类写入 docs/项目文档/配置归类-分组映射.csv
 * 运行：node scripts/apply-giftcard-settings-mapping.mjs
 */
...
const giftcardAssign = new Map();
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    giftcardAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
for (const line of lines) {
  ...
  const next = giftcardAssign.get(seq);
  if (next) {
...
if (updated !== giftcardAssign.size) {
  throw new Error(`预期更新 ${giftcardAssign.size} 条，实际 ${updated} 条`);
}
