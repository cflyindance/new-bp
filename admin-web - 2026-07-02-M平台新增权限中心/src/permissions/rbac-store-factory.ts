...
export function createMerchantSeedRoles(): RbacRole[] {
  ...
  return [
    roleFromGrantsSeed("store-manager", "店长", "门店全量管理，含财务与报表操作", true, all, "pos"),
    roleFromGrantsSeed("hq-admin", "总部管理员", "连锁总部：品牌/门店/权限/多店汇总", true, all, "pos"),
    roleFromGrantsSeed(
      "cashier",
      "收银员",
...
export interface RbacStoreApi {
  ...
  updateStaffAssignments: (staff: StaffAssignment[]) => void;
