...
export function createStaffAccountStore(options: CreateStaffAccountStoreOptions): StaffAccountStoreApi {
  ...
  function mergeSeedAccounts(loaded: StaffLoginAccount[]): StaffLoginAccount[] {
    const seeded = options.seedAccounts();
    const byEmail = new Map(loaded.map((a) => [normalizeEmail(a.loginEmail), a]));
    let changed = false;
    for (const seed of seeded) {
      const email = normalizeEmail(seed.loginEmail);
      const existing = byEmail.get(email);
      if (!existing) {
        byEmail.set(email, seed);
        changed = true;
        continue;
      }
      const patch: Partial<StaffLoginAccount> = {};
      if (seed.orgTier && !existing.orgTier) patch.orgTier = seed.orgTier;
      if (seed.groupId && !existing.groupId) patch.groupId = seed.groupId;
      if (seed.anchorMerchantId && !existing.anchorMerchantId) patch.anchorMerchantId = seed.anchorMerchantId;
      if (Object.keys(patch).length) {
        byEmail.set(email, { ...existing, ...patch, updatedAt: new Date().toISOString() });
        changed = true;
      }
    }
  ...
  function loadAccounts(): StaffLoginAccount[] {
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed) && parsed.length) return mergeSeedAccounts(parsed);
      }
    } catch {
      /* ignore */
    }
