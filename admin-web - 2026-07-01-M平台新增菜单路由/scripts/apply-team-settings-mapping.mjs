...
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    teamAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
if (updated !== teamAssign.size) {
  throw new Error(`预期更新 ${teamAssign.size} 条，实际 ${updated} 条`);
}
