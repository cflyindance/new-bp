...
function buildCatalog(rows, mapping) {
  ...
  ts += `}\n\n`;
  ts += `export function slugifyModuleSettingsGroupKey(groupKey: string): string {\n`;
  ts += `  return (\n`;
  ts += `    groupKey\n`;
  ts += `      .toLowerCase()\n`;
  ts += `      .replace(/[^\\w\\u4e00-\\u9fff]+/g, "-")\n`;
  ...
  ts += `  );\n`;
  ts += `}\n\n`;
  ts += `export function getModuleSettingsCategoryPath(settingsPath: string, groupKey: string): string {\n`;
  ts += `  return \`\${settingsPath}/\${encodeURIComponent(slugifyModuleSettingsGroupKey(groupKey))}\`;\n`;
  ts += `}\n\n`;
  ts += `export function getModuleSettingsItemHref(\n`;
  ts += `  settingsPath: string,\n`;
  ts += `  item: Pick<ModuleSettingCatalogItem, "groupKey" | "id">,\n`;
  ts += `): string {\n`;
  ts += `  return \`\${getModuleSettingsCategoryPath(settingsPath, item.groupKey)}#\${item.id}\`;\n`;
  ts += `}\n\n`;
  ts += `export function listAllModuleSettingCatalogEntries(): Array<{\n`;
  ts += `  hubTitle: string;\n`;
  ts += `  settingsPath: string;\n`;
  ts += `  item: ModuleSettingCatalogItem;\n`;
  ts += `}> {\n`;
  ts += `  const rows: Array<{ hubTitle: string; settingsPath: string; item: ModuleSettingCatalogItem }> = [];\n`;
  ts += `  for (const hub of Object.values(MODULE_SETTINGS_BY_PATH)) {\n`;
  ts += `    for (const item of hub.items) {\n`;
  ts += `      rows.push({ hubTitle: hub.hubTitle, settingsPath: hub.settingsPath, item });\n`;
  ts += `    }\n`;
  ts += `  }\n`;
  ts += `  return rows;\n`;
  ts += `}\n\n`;
