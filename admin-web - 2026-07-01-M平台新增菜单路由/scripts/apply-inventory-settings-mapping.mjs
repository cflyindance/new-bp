/**
 * 将库存管理中心 5 组分类写入 docs/项目文档/配置归类-分组映射.csv
 * 运行：node scripts/apply-inventory-settings-mapping.mjs
 */
...
const invAssign = new Map();
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    invAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
if (updated !== invAssign.size) {
  throw new Error(`预期更新 ${invAssign.size} 条，实际 ${updated} 条`);
}
