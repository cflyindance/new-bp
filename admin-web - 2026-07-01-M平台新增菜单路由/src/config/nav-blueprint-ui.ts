...
import {
  ...
  countCustomAssignedSeqs,

  countSystemAssignedSeqs,
  ...
  return `
    ...
    <section class="rounded-xl border border-border bg-card px-4 py-4 shadow-sm">
      ...
      <p class="mt-1 text-xs text-muted-foreground">
        ...
        系统预设与自定义导航可分别配置；发布后将按此处选择决定商家侧使用的导航树结构。
      ...
      <fieldset class="mt-3 space-y-2">
        ...
        <label class="flex items-start gap-2 text-sm">
          ...
          <input type="radio" name="nb-navigation-source" value="system" class="mt-0.5" ${isSystem ? "checked" : ""} />

          <span>

            <span class="font-medium text-card-foreground">系统默认导航</span>

            <span class="mt-0.5 block text-xs text-muted-foreground">下方展示系统预设导航树，可调整启用态与设置项归属</span>
        ...
        <label class="flex items-start gap-2 text-sm">
          ...
          <input type="radio" name="nb-navigation-source" value="custom" class="mt-0.5" ${!isSystem ? "checked" : ""} />

          <span>

            <span class="font-medium text-card-foreground">自定义导航树</span>

            <span class="mt-0.5 block text-xs text-muted-foreground">下方展示自定义导航树，可创建一级 / 二级 / 三级导航</span>
      ...
      <p class="mt-3 text-xs text-muted-foreground">
        ...
        切换来源后，下方仅展示对应导航树的结构目录（另一模块配置仍保留，发布时按此处选择生效）。
...
function renderTreeModulePanel(
  ...
  const enabledL1 = countEnabledL1(selection, snapshot, module);
  ...
  const subtitle = isSystem
    ...
    ? "控制页面与设置展示；三级分组按 B 端侧栏分类（如前厅 员工端/食客端），不含操作按钮权限"

    : "企业自建导航结构；手动配置二级类一级先新增二级，选中二级后再新增三级分组；「页面」「设置/功能」类一级不展示新增按钮";

  const assignedCount = isSystem

    ? countSystemAssignedSeqs(snapshot)

    : countCustomAssignedSeqs(snapshot);

  const customCount = isSystem ? 0 : countCustomNodes(snapshot);
  ...
  const stats = isSystem
    ...
    ? `已启用 <strong class="text-card-foreground">${enabledL1}</strong> 个一级 · 设置重归属 <strong class="text-card-foreground">${assignedCount}</strong> 项`

    : `自定义节点 <strong class="text-card-foreground">${customCount}</strong> 个 · 已启用 <strong class="text-card-foreground">${enabledL1}</strong> 个一级 · 设置归属 <strong class="text-card-foreground">${assignedCount}</strong> 项`;
    ...
    >
      ...
      <div class="border-b border-border px-4 py-3 flex flex-wrap items-start justify-between gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="text-xs text-muted-foreground mt-0.5">${escapeHtml(subtitle)}</p>

          <p class="text-xs text-muted-foreground mt-1">${stats} · ${isSystem ? "业务页勾选二级即可；设置类入口可配置四级设置项归属" : "四级列勾选表示将设置归属到当前三级分组"}</p>
  ...
  return `
    ...
    >
      ...
      <div class="rounded-xl border border-sky-500/25 bg-sky-500/10 px-4 py-3 text-sm text-muted-foreground">
        ...
        选择<strong class="text-card-foreground">导航默认来源</strong>后，下方仅展示对应导航树的结构目录；两个模块配置互不影响，发布时按所选来源生效。
