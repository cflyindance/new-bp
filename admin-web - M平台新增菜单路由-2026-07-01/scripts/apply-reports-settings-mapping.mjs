/**
 * 将报表中心 3 组分类写入 docs/项目文档/配置归类-分组映射.csv
 * 运行：node scripts/apply-reports-settings-mapping.mjs
 */
...
const reportsAssign = new Map();
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    reportsAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
for (const line of lines) {
  ...
  const next = reportsAssign.get(seq);
  if (next) {
...
if (updated !== reportsAssign.size) {
  throw new Error(`预期更新 ${reportsAssign.size} 条，实际 ${updated} 条`);
}
