/**
 * 将预约等位中心 3 组分类写入 docs/项目文档/配置归类-分组映射.csv
 * 运行：node scripts/apply-reservation-settings-mapping.mjs
 */
...
const reservationAssign = new Map();
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    reservationAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
for (const line of lines) {
  ...
  const next = reservationAssign.get(seq);
  if (next) {
...
if (updated !== reservationAssign.size) {
  throw new Error(`预期更新 ${reservationAssign.size} 条，实际 ${updated} 条`);
}
