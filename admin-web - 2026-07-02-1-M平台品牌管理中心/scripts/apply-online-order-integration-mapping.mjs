/**
 * 线上订餐技术对接（原外卖/来取 online-integration）→ 平台业务中心
 * 运行：node scripts/apply-online-order-integration-mapping.mjs
 */
...
const titles = {
  "online-order-service": "线上订餐服务对接",
};

const assignMap = {
  "online-order-service": [93, 95, 96, 97, 99, 100, 101, 102, 103, 104, 105, 106],
};

const integrationAssign = new Map();
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    integrationAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
for (const line of lines) {
  ...
  const next = integrationAssign.get(seq);
  if (next) {
...
if (updated !== integrationAssign.size) {
  throw new Error(`预期更新 ${integrationAssign.size} 条，实际 ${updated} 条`);
}
