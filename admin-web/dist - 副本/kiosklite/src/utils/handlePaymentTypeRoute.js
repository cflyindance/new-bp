...
import { shouldStartTipBeforePaymentMethod } from './tipProcedure';
...
const handlePaymentTypeRoute = (systemConfig, selfConfig) => {
  ...
  const paymentTypes = resolveKioskPaymentTypes(selfConfig, systemConfig);
  ...
  const canPayByCard = paymentTypes.includes('0') && devicePayByCard;
  const canPayByCash = paymentTypes.includes('1') && devicePayByCash;
  const canPayByEcard = paymentTypes.includes('2') && devicePayByEcard;
  ...
  const shouldKeepPaymentTypeForTipFlow = shouldStartTipBeforePaymentMethod(
    selfConfig,
    systemConfig
  );
  const shouldSkipPaymentType =
    (onlyCard || onlyCash) && !shouldKeepPaymentTypeForTipFlow;
