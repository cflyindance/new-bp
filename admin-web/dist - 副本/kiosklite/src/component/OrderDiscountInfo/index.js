...
const OrderDiscountInfo = (props) => {
  ...
  const promotionHeader = useMemo(() => {
    const selectedItem = Array.isArray(promotionItem)
      ? promotionItem[0]
      : promotionItem;
    const selectedName = selectedItem?.displayName;
    if (selectedName) {
      return Array.isArray(selectedName) ? selectedName[0] : selectedName;
    }
  ...
  const groupedData = useMemo(() => {
    ...
    arrays.forEach(({ list, type }) => {
      ...
      const firstRuleText = list[0]?.ruleText ?? list[0]?.text;
      if (
        list.length > 1 ||
        (list.length === 1 &&
          Array.isArray(firstRuleText) &&
          firstRuleText.length > 1)
      ) {
        result.push({
          type: 'group',
          title: t(`${type}_title`),
        ...
        list.forEach((item, idx) => {
          const text = item?.ruleText ?? item?.text;
        ...
        const text = list[0]?.ruleText ?? list[0]?.text;
