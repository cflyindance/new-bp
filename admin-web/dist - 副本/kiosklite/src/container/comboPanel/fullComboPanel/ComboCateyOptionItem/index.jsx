...
import styles from './ComboCateyOptionItem.module.scss';
import { connect } from 'react-redux';
import { withTranslation } from 'react-i18next';
import { getDishItemLanguage, allRangHandler } from '@/utils/busTools';
...
const ComboCateyOptionItem = (props) => {
  ...
  const getNoSubList = () => {
    let sub = [];
    let pri = [];
    currentItem?.categoryOptions?.forEach((item) => {
  ...
  const getCurrentItem = () => {
    const cateOpt = currentOrderCombo?.find((c) => c.id === -3);
    if (!cateOpt || !cateOpt.options) return;
    // 使用 reduce 方法统计每个 id 的出现次数
    const countById = cateOpt.options.reduce((acc, item) => {
  ...
  useEffect(() => {
    setChoseTipMap(
      allRangHandler(sideNavList, t, { isInFreeItem, isPromotionItem })
    );
  ...
  const handleClick = (obj, mode = 1) => {
    const currentCheckCate = currentItem?.categoryOptions?.find(
    ...
    if (obj.itemOptionId) {
      ...
      if (checkCount && isCurrentIdMatched) {
        ...
        props.removeOneOption({ sideNavId: -3, id: obj.id });
      } else if (!checkCount) {
        // 选中二级option
      ...
      if (mode === 1) {
        ...
        if (checkCount && isCurrentIdMatched) {
          ...
          props.addOption(-3, { ...obj, price, quantity: 1 });
        } else {
          // 未选中时
          if (!handleOptionAdd(1)) return; // 检查是否超过最大限制
          updateCheckedMapAndIds(1, obj.id);
  ...
  const renderItemCard = (item, isActive, isDisabled, onClick) => {
    ...
    return (
      <div
        ...
        style={
          !item.itemOptionId && isVertical && isShowComboSideNav
  ...
  const renderNoSubOptions = () => (
    <div className={styles.cateyBoxItem}>
  ...
  const renderSubOptions = (itemInfo) => {
    ...
    return (
      <div key={itemInfo.id} className={styles.cateyBoxItem}>
  ...
  // 主组件的返回部分
  return (
    (hasSubList.length > 0 || hasNoSubList.length > 0) && (
      <div className={styles.cateyBox}>
...
export default connect(mapStateToProps, { addOption, removeOneOption })(
  withTranslation()(ComboCateyOptionItem)
);
