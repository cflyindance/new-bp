...
import { isTipBeforePaymentMethodFlow } from '@/utils/tipProcedure';
...
class TippingPanel extends React.Component {
  ...
  isBeforePaymentMethodFlow = () =>
    isTipBeforePaymentMethodFlow({
      selfConfig: this.props.selfConfig,
      locationState: this.props.location?.state,
      tipFlowState: this.props.currentOrder?.tipFlowState,
    });

  backBtnHandler = () => {
    if (this.isBeforePaymentMethodFlow()) {
      this.props.saveTipAmount(0);
      this.props.saveTipFlowState({
  ...
  handleConfirmPay = () => {
    ...
    this.setState(
      ...
      () => {
        ...
        const selection = this.state.customTip
          ? { kind: 'custom', value: this.state.tipValue }
          : this.state.tipValue === 0
            ? { kind: 'none', value: 0 }
            : { kind: 'percentage', value: this.state.tipValue };
        if (this.isBeforePaymentMethodFlow()) {
          this.props.saveTipFlowState({
            completedBeforePaymentMethod: true,
            selection,
          });
        }
        // 已输入小费，再判断售罄情况，再判断是否符合刷卡最低消费
        this.judgeConfigToSoldout(this.continueAfterTip);
      }
    );
  };
  ...
  continueAfterTip = () => {
    if (this.isBeforePaymentMethodFlow()) {
      this.props.history.push('/paymentType', {
        tipBeforePaymentMethodCompleted: true,
      });
      return;
    }
    this.judgeFillCardMinAmout();
  };
  ...
  componentDidMount() {
    if (this.isBeforePaymentMethodFlow()) {
      const selection = this.props.currentOrder?.tipFlowState?.selection;
      if (selection?.kind === 'custom') {
        this.setState({
          customTip: true,
          tipValue: selection.value,
          customNumber: selection.value,
        });
      } else if (selection?.kind === 'none') {
        this.setState({ customTip: false, tipValue: 0 });
      } else if (selection?.kind === 'percentage') {
        this.setState({ customTip: false, tipValue: selection.value });
      }
    }
    // 横屏模式下监听原生键盘弹出
