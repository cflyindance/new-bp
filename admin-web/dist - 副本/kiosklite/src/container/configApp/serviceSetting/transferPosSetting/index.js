...
import { getKioskPaymentTypesConfigState } from '@/utils/kioskPaymentTypes';
import { getPosConfigSaveDecision } from '@/utils/posConfigSavePolicy';
...
class TransferPosSetting extends Component {
  constructor() {
    ...
    this.state = {
      ...
      posConfigLoaded: false,
    };
  }

  componentDidUpdate(prevProps) {
    const previousValue = JSON.stringify(prevProps.paymentTypesConfig?.value);
    const currentValue = JSON.stringify(this.props.paymentTypesConfig?.value);
    if (previousValue === currentValue || !this.state.posConfigLoaded) return;

    const posConfigList = this.applyPersistedPaymentTypes(
      cloneDeep(this.state.posConfigList)
    );
    this.setState({ posConfigList }, () => {
      const paymentType = posConfigList.find(
        (item) => item['app:name'] === 'KIOSK_PAYMENT_TYPE'
      )?.['app:value'];
      this.props.onPaymentTypeChange?.(paymentType, { loaded: true });
    });
  }

  applyPersistedPaymentTypes = (list) => {
    const configState = getKioskPaymentTypesConfigState(
      this.props.paymentTypesConfig ? [this.props.paymentTypesConfig] : []
    );
    if (configState.status === 'missing') return list;
    const paymentConfig = list.find(
      (item) => item['app:name'] === 'KIOSK_PAYMENT_TYPE'
    );
    if (paymentConfig) paymentConfig['app:value'] = configState.value.join(',');
    return list;
  };
  ...
  handleCheckBox = (id, e) => {
    ...
    this.setState(
      ...
      () => {
        const paymentType = this.state.posConfigList?.find(
          (item) => item['app:name'] === 'KIOSK_PAYMENT_TYPE'
        )?.['app:value'];
        this.props.onPaymentTypeChange?.(paymentType, { source: 'user' });
      }
    );
  };
  ...
  getSaveDecision = () => {
    ...
    return getPosConfigSaveDecision({
      current: posConfigList,
      original: originalPosConfigList,
      userId,
    });
  };

  confirmPaymentTypeSync = async () =>
    Modal.loadModal(<SubmitModal />, {
      width: '500px',
      footer: null,
      title: null,
      maskClosable: false,
  ...
  savePosDetail = async () => {
    ...
    const saveDecision = this.getSaveDecision();
    if (!saveDecision.shouldSavePos) {
      return Promise.resolve();
    }
    let str = '';
    return new Promise((resolve, reject) => {
          ...
          saveKioskConfigFromPos(str, userId).then((res) => {
            ...
            try {
              ...
              if (r.successful === 'true') {
                ...
                reject(new Error(r?.failureReason || 'save POS config failed'));
              }
            } catch (err) {
              reject(err);
            }
      ...
      promiseFinally(
        getKioskConfigFromPos()
          .then((res) => {
            ...
            const rawPosConfigList = cloneDeep(list);
            const resolvedList = this.applyPersistedPaymentTypes(
              cloneDeep(list)
            );
            this.setState({
              posConfigList: cloneDeep(resolvedList),
              originalPosConfigList: rawPosConfigList,
              posConfigLoaded: true,
            });
            const paymentType = resolvedList?.find(
              (item) => item['app:name'] === 'KIOSK_PAYMENT_TYPE'
            )?.['app:value'];
            this.props.onPaymentTypeChange?.(paymentType, { loaded: true });
            resolve(res.data);
