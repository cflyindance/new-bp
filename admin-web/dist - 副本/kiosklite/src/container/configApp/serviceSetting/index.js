...
class ServiceSetting extends Component {
  ...
  handlePaymentTypeChange = (paymentType, options = {}) => {
    this.setState((state) => {
      let configList = cloneDeep(state.configList);
      const persistedState = getKioskPaymentTypesConfigState(configList);
      const incomingPaymentTypes = normalizeKioskPaymentTypes(
        String(paymentType ?? '').split(',')
      );
      const authoritativeLoad = options.loaded && persistedState.status !== 'missing';
      const selectedPaymentTypes = authoritativeLoad
        ? persistedState.value
        : incomingPaymentTypes;
      if (selectedPaymentTypes.length) {
        configList = upsertKioskPaymentTypesConfig(
          configList,
          selectedPaymentTypes
        );
      }
      const availableTipPaymentTypes = normalizePaymentTypes(
        selectedPaymentTypes
      );
      const tipIndex = configList.findIndex((item) => item.id === 5);
      if (tipIndex > -1) {
        configList[tipIndex] = reconcileTipConfig(
          configList[tipIndex],
          availableTipPaymentTypes
        );
      }
      const paymentConfigLoaded = options.loaded || state.posPaymentConfigLoaded;
      return {
        configList: reconcileTipProcedure(
          configList,
          availableTipPaymentTypes,
          paymentConfigLoaded
        ),
        availableTipPaymentTypes,
        posPaymentConfigLoaded: paymentConfigLoaded,
      };
    });
  };

  handleTipPaymentTypeChange = (paymentType) => {
    const { availableTipPaymentTypes, configList } = this.state;
    if (!availableTipPaymentTypes.includes(String(paymentType))) return;
    const newConfig = cloneDeep(configList);
    const tipIndex = newConfig.findIndex((item) => item.id === 5);
    if (tipIndex < 0) return;
    const selected = Array.isArray(newConfig[tipIndex].tipPaymentTypes)
      ? normalizePaymentTypes(newConfig[tipIndex].tipPaymentTypes)
      : [...availableTipPaymentTypes];
    const code = String(paymentType);
    newConfig[tipIndex].tipPaymentTypes = selected.includes(code)
    ...
    this.setState({
      configList: reconcileTipProcedure(
        newConfig,
        availableTipPaymentTypes,
        this.state.posPaymentConfigLoaded
      ),
    });
  };
  ...
  handleChangeRadio = (id, e) => {
    ...
    if (id === 24) {
      const eligibleTipProcedures = getEligibleTipProcedures(
        configList.find((item) => item.id === 5),
        this.state.availableTipPaymentTypes,
        this.state.posPaymentConfigLoaded
      );
      if (!eligibleTipProcedures.includes(e)) return false;
    }
    let i = configList?.findIndex((c) => c.id == id);
    if (i > -1) {
      if (id == 8 || id == 9) {
      ...
      getMarginappFetchKioskConfig(event.data.data)
        .then(async (res) => {
          if (res.data.result.successful) {
            ...
            const normalizedTipProcedure = Number(tipsConfig?.value);
            const showAuthForTips =
              isOpenDualPrice &&
              [0, 2].includes(normalizedTipProcedure) &&
            ...
            if (this.state.posPaymentConfigLoaded) {
              const tipIndex = configList.findIndex((item) => item.id === 5);
              if (tipIndex > -1) {
                configList[tipIndex] = reconcileTipConfig(
                  configList[tipIndex],
              ...
              const tipProcedure = configList.find((item) => item.id === 24);
              const eligibleTipProcedures = getEligibleTipProcedures(
                configList.find((item) => item.id === 5),
                this.state.availableTipPaymentTypes,
                true
              ...
              if (
                eligibleTipProcedures.length > 0 &&
                !eligibleTipProcedures.includes(
                  Number(tipProcedure?.value)
                )
              ) {
                Toast.info(
                  t('tip-procedure-required', {
                    defaultValue: 'Please select a valid tip flow.',
                  }),
                  2000
                );
                return false;
            ...
            const paymentTypeValue =
              this.posDetail?.state?.posConfigList?.find(
                (item) => item['app:name'] === 'KIOSK_PAYMENT_TYPE'
              )?.['app:value'];
            const selectedPaymentTypes = normalizeKioskPaymentTypes(
              String(paymentTypeValue ?? '').split(',')
            );
            if (!selectedPaymentTypes.length) {
              Toast.info(t('pay-tip'), 1000);
              return false;
            }
            params.configList = upsertKioskPaymentTypesConfig(
              configList,
              selectedPaymentTypes
            );
            ...
            if (posSaveDecision.blocked) {
              Toast.info(
                t('pos-config-admin-auth-required', {
                  defaultValue:
                    'Administrator authentication is required to save order type or SMS settings.',
                }),
                3000
              );
              return;
            }
            if (posSaveDecision.requiresPaymentConfirmation) {
              const confirmed = await this.posDetail?.confirmPaymentTypeSync();
              if (!confirmed) return;
            }
            ...
            if (!marginRes?.data?.result?.successful) {
              this.setState({ msg: 'FAIL', open: true });
              return;
            }

            this.setState({ msg: 'SUCCESS', open: true });
            if (posSaveDecision.shouldSavePos) {
              try {
                await this.posDetail?.savePosDetail();
              } catch (error) {
                Toast.info(
                  t('payment-method-pos-sync-failed', {
                    defaultValue:
                      'Payment methods were saved, but POS compatibility sync failed. Please retry.',
                  }),
                  3000
                );
              }
    ...
    configList.map((c) => {
      ...
      let com = (
        <ServiceItem
          ...
          availableTipPaymentTypes={availableTipPaymentTypes}
          posPaymentConfigLoaded={posPaymentConfigLoaded}
          handleTipPaymentTypeChange={this.handleTipPaymentTypeChange}
          {...(c.id === 61
          ...
          >
            ...
            <div className={styles.rightContent}>
              <TransferPosSetting
                ...
                paymentTypesConfig={configList.find((item) => item.id === 70)}
                leftCategoryVal={leftCategoryVal}
              />
            </div>
          </div>
