...
| 5 | `tipping-mode` | 显示小费选项 | `true`（兼容字段） | 小费展示由 `tipPaymentTypes` 控制；`value` 不再参与新版运行时判断 |
...
| 24 | `tip-procedure` | 选择小费流程 | `0` | 0-刷卡之前, 1-刷卡之后, 2-支付方式选择页面之前 |
...
| 64 | `promotion-center-activity-name` | 促销中心活动标签标题来源 | `0` | 0-系统默认, 1-活动名称, 2-活动标签；统一控制 Kiosk 本地促销与促销中心活动的食客端识别名称 |
...
- 同时影响 **Kiosk 本地促销** 与 **促销中心活动** 的食客端活动识别名称；金额、数量、折扣和赠品等交易条件仍使用规则文案。
- `0`（系统默认）：沿用 `activityRule` 生成的规则文案（与改配置前行为一致）。
- `1`（活动名称）：本地促销按 Kiosk 语言取 `activityTitle`；促销中心活动取 `promotionName`。
- `2`（活动标签）：本地促销按 Kiosk 语言取 `activityTag`；促销中心无独立标签字段，回退 `promotionName`。
- 中文语系（包括简体、繁体）取 `zh`，所有非中文语言取 `en`；目标字段缺失或为空时回退规则文案。
...
### 促销中心活动标签标题来源值
- `0`: 系统默认（使用活动规则生成的文案）
- `1`: 活动名称（本地取双语 `activityTitle`；促销中心取 `promotionName`）
- `2`: 活动标签（本地取双语 `activityTag`；促销中心回退 `promotionName`）
...
| `tipPaymentTypes` | `('0'|'1'|'2')[]` | id 5 专用；允许展示小费的支付方式，`0` 为信用卡/借记卡、`1` 为柜台支付、`2` 为礼品卡 |
...
```json
{
  ...
  "key": "tipping-mode",
  "value": true,
  "tipPaymentTypes": ["0", "1", "2"]
}
```

| 字段/值 | 含义 |
|---|---|
| `value` | 旧接口兼容布尔字段；设置页不展示总开关，新版食客端不读取它决定是否显示小费 |
| `tipPaymentTypes: ['0']` | 仅信用卡/借记卡展示小费 |
| `tipPaymentTypes: ['1']` | 仅柜台支付展示小费 |
| `tipPaymentTypes: ['0', '1']` | 两种支付方式都展示小费 |
| `tipPaymentTypes: ['2']` | 仅礼品卡展示小费 |
| `tipPaymentTypes: ['0', '1', '2']` | 三种支付方式都展示小费 |
| `tipPaymentTypes: []` | 所有支付方式都不展示小费；显式空数组不得回填默认值 |

- `tipPaymentTypes` 缺失、为 `null` 或类型非法时按旧数据处理：首次取得确定的 `KIOSK_PAYMENT_TYPE` 后，默认迁移为其中已启用的信用卡/柜台/礼品卡支付方式。
- 数组输入会过滤重复值、未知值和支付设置中未启用的方式。
- 设置页默认、常驻展示“信用卡/借记卡”“柜台支付”“礼品卡”；只有 `KIOSK_PAYMENT_TYPE` 已启用的对应项才可勾选。
- 支付设置取消某方式时，对应小费选项立即自动取消；重新启用时只恢复可选，不自动勾选。
- 礼品卡属于 `tipPaymentTypes`，并使用代码 `2`。
...
- 值 `0`、`1` 仅在 `KIOSK_PAYMENT_TYPE` 与 id 5 的 `tipPaymentTypes` 都包含 `0` 时允许选择。
- 值 `2` 仅在 `KIOSK_PAYMENT_TYPE` 至少包含两种方式，且其中每一种都包含在 id 5 的 `tipPaymentTypes` 中时允许选择。
- 当前值失效时优先回退值 `0`；若值 `0` 无效但仍有其他有效流程，则清空并要求用户重新选择后才能保存；三个流程均无效时清空且全部置灰。
- 值 `2` 已选后不再满足“至少两种支付方式且所有已启用方式都勾选小费”时，按上一条规则回退或清空，并在统一保存时持久化。
- 点单运行时对数字 `2` 与字符串 `'2'` 做同义规范化。只要读取到值 `2`，必须执行
  `/tippingPanel → /paymentType → 所选支付流程`；运行时不得再次用支付能力或小费选项把它降级为“小费页 → 直接信用卡支付”。
- 值 `2` 时，小费页确认仅保存小费及完成状态并跳转 `/paymentType`；信用卡最低消费校验和进入 `/cardPayment` 的业务由支付方式页信用卡按钮执行。柜台支付按钮沿用既有确认框、建单与完成逻辑。

#### 值 2 使用的订单运行时状态（不持久化到 MarginApp）

`currentOrder.tipFlowState` 是 Redux 订单态，不属于本文件开头定义的 MarginApp `data`：

| 路径 | 类型 | 含义 |
|---|---|---|
| `tipFlowState.completedBeforePaymentMethod` | `boolean` | 是否已经在支付方式选择页之前确认过小费；支付方式按钮用它防止绕过或重复进入小费页 |
| `tipFlowState.selection` | `object\|null` | 小费选择元数据，用于返回页面恢复选择 |
| `tipFlowState.selection.kind` | `percentage\|custom\|none` | 预设比例、自定义金额或 No Tip |
| `tipFlowState.selection.value` | `number` | 原始百分比或自定义金额；No Tip 为 `0` |

小费页确认时同步更新订单 `tipAmount`、`tipFlowState.selection` 和完成标志；返回并取消本轮结账、新订单、重下单、订单取消或完成清理时必须清除，不能跨订单复用。
...
| `activityTitle` | `{zh: string, en: string}` | 活动标题；中英文均必填 |
| `activityTag` | `{zh: string, en: string}` | 活动标签；中英文均必填 |
...
`activityTitle`、`activityTag` 保存前会去除首尾空格。旧活动缺少这些字段时仍可读取和生效，食客端回退规则文案；旧活动再次编辑保存时必须补齐四个语言值。
