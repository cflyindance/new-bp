...
import type { ProductSeasoningRelation, SeasoningActionCode, SeasoningOption, SeasoningOptionCategory } from "./seasoning-types";
...
export function createConfiguredOption(optionId: string): SeasoningConfiguredOption {
  return { optionId, ...createBatchOptionPricing() };
}
...
function renderActionPicker(view: SeasoningWorkspaceView): string {
  ...
  return `<div class="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-[2px]" data-picker-backdrop="action"><section role="dialog" aria-modal="true" aria-labelledby="seasoning-action-picker-title" class="w-full max-w-2xl rounded-2xl border border-border bg-card p-5 shadow-2xl"><div class="flex items-start justify-between gap-4"><h3 id="seasoning-action-picker-title" class="text-lg font-semibold">${t("seasoning.batch.addAction")}</h3><button type="button" data-close-picker="action" class="${secondaryButtonClass}" aria-label="${t("seasoning.close")}">×</button></div><div class="mt-5 grid gap-3 sm:grid-cols-2">${SEASONING_ACTIONS.map((definition) => {
...
export function renderSeasoningConfigurationWorkspace(view: SeasoningWorkspaceView): string {
  ...
  const columns = "grid-cols-[28px_36px_minmax(140px,1fr)_130px_110px_140px_44px]";
