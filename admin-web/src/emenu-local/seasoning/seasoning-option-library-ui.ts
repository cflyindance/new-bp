...
import type { SeasoningOption } from "./seasoning-types";
...
export function openSeasoningOptionEditor(
  host: HTMLElement,
  input: { version: number; option?: SeasoningOption; onSaved: (version: number) => Promise<void> | void },
): void {
  ...
  overlay.querySelector<HTMLFormElement>("[data-seasoning-option-form]")?.addEventListener("submit", async (event) => {
    ...
    try {
      const payload = { expectedVersion: input.version, name: String(data.get("name") ?? ""), nameEn: String(data.get("nameEn") ?? ""), code: String(data.get("code") ?? ""), sortOrder: Number(data.get("sortOrder") ?? 10) };
