...
import {
  bindFohBrandMenuUi,
  isFohBrandMenuPath,
  renderFohBrandMenuPage,
  FOH_BRAND_MENU_PATH,
} from "./config/foh-brand-menu-settings-ui";
import {
...
import {
  ...
  ensurePosOrderCartPosLinesDefault,
...
import {
  ...
  ensureDefaultMainScreenLinesDefault,
  ensureDefaultMainScreenToggleMigrated,
  isDefaultMainScreenSeq,
  renderDefaultMainScreenPanelHtml,
  renderPosShellLandingGroupIntroHtml,
  setDefaultMainScreenPanelVisible,
...
} from "./config/module-settings-scene-desc-help-ui";
import {
...
function isTeamRolesEmployeesPath(path: string): boolean {
...
/** 团队管理原生员工与岗位页：保留主项目导航和账号栏，不加载 TipOut iframe。 */
function renderTeamRolesEmployeesPanel(): string {
  ensureInPageDefaultStoreSelected();
  syncScopeFilterMetaForEmbeddedPages();
  return `
...
function normalizeTabModuleHashes(): void {
  ...
  if (
    raw === `${FOH_CATEGORY_SETTINGS_BASE}/hours` ||
    raw.startsWith(`${FOH_CATEGORY_SETTINGS_BASE}/hours/`)
  ) {
    replaceHashPath(getFohCategorySettingsTabPath("age-category"));
    return;
  }
  if (
    raw === `${FOH_CLASSIFICATION_SETTINGS_BASE}/category` ||
    raw === `${FOH_CLASSIFICATION_SETTINGS_BASE}/menu` ||
    raw === `${FOH_CLASSIFICATION_SETTINGS_BASE}/hours` ||
    raw.startsWith(`${FOH_CLASSIFICATION_SETTINGS_BASE}/category/`) ||
    raw.startsWith(`${FOH_CLASSIFICATION_SETTINGS_BASE}/menu/`) ||
    raw.startsWith(`${FOH_CLASSIFICATION_SETTINGS_BASE}/hours/`)
  ) {
    replaceHashPath(getFohClassificationSettingsTabPath("category"));
    return;
  }
  /* 品类与场景菜单已迁至「品类管理」功能页，旧设置分组书签重定向 */
  if (
  ...
  if (raw === "/team/reports/overview" || raw.startsWith("/team/reports/overview/")) {
    replaceHashPath("/team/reports/summary");
    return;
  }
  if (raw === "/team/reports/tips" || raw.startsWith("/team/reports/tips/")) {
    replaceHashPath("/team/reports/tip-summary");
    return;
  }
...
function renderSidebarBrandHeader(): string {
  const showGroupSwitcher = shouldShowMerchantGroupSwitcher();
  const groups = listMPlatformGroupsForMerchantBackend();
  const ctx = showGroupSwitcher ? resolveChainBrandContext() : null;
  const activeGroupId = ctx?.groupId ?? "";
  const activeGroupName = groups.find((g) => g.groupId === activeGroupId)?.name ?? t("shell.appName");
  const tagline = showGroupSwitcher ? t("shell.groupSwitchTagline") : t("shell.appTagline");

  if (showGroupSwitcher && groups.length > 1) {
    return `
        <div class="min-w-0 flex-1">
          <label class="sr-only" for="sidebar-merchant-group-switch">${escapeHtml(t("shell.groupSwitchAria"))}</label>
          <select
            id="sidebar-merchant-group-switch"
            data-merchant-group-switch
            class="w-full min-w-0 truncate rounded-md border border-sidebar-border bg-sidebar px-1.5 py-1 text-sm font-semibold tracking-tight text-sidebar-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-active"
            aria-label="${escapeHtml(t("shell.groupSwitchAria"))}"
          >
            ${groups
              .map(
                (g) =>
                  `<option value="${escapeHtml(g.groupId)}"${g.groupId === activeGroupId ? " selected" : ""}>${escapeHtml(g.name)}</option>`,
              )
              .join("")}
          </select>
          <p class="mt-0.5 truncate text-xs text-sidebar-muted">${escapeHtml(tagline)}</p>
        </div>`;
  }

  const title = showGroupSwitcher ? activeGroupName : t("shell.appName");
  return `
        <div class="min-w-0 flex-1">
          <a href="#${APP_NAV_HOME_PATH}" class="block min-w-0 rounded-md transition-colors hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-active focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar">
            <p class="truncate text-sm font-semibold tracking-tight ${SBR_TITLE}">${escapeHtml(title)}</p>
            <p class="truncate text-xs text-sidebar-muted">${escapeHtml(tagline)}</p>
          </a>
        </div>`;
}

function bindSidebarGroupSwitcher(): void {
  const el = document.querySelector<HTMLSelectElement>("[data-merchant-group-switch]");
  if (!el) return;
  el.addEventListener("change", () => {
    const groupId = el.value;
    if (!groupId || groupId === resolveChainBrandContext()?.groupId) return;
    writeActiveMerchantGroupId(groupId);
    resetScopeFiltersForGroupChange();
    mount();
  });
}
...
function isModuleHubSettingsCatalogPath(path: string): boolean {
  ...
  if (isFohBrandMenuPath(path)) return false;
...
function renderTeamShiftSchedulingPageWithSettings(): string {
  const items = getTeamSettingItemsBySeqs(TEAM_SHIFT_SCHEDULING_SETTING_SEQS);
  const rulesPanel = renderTeamSettingsTabPanel({
    description: "控制打卡是否必须匹配当日排班；与员工打卡页联动。",
    rowsHtml: renderModuleSettingRowsHtml(items),
  });
  return wrapPageWithSaveBar("/team/shift-scheduling", renderTeamShiftSchedulingPage(rulesPanel));
}
...
function renderTeamClockInPageWithSettings(): string {
  ...
  const rulesPanel = renderTeamSettingsTabPanel({
    description: "按开收工、登出门禁与打卡凭证分组配置；修改后需保存并下发。",
    sectionsHtml: renderTeamClockInRulesSectionsHtml(items),
  });
  const page = renderTeamClockInPage(rulesPanel);
  // 考勤记录 / 实时打卡不需要底部保存栏；规则设置仍走页脚保存并下发
  if (!shouldShowTeamClockInSaveBar()) return page;
  return wrapPageWithSaveBar("/team/clock-in", page);
}

function renderFohBrandMenuGroupCard(
  groupTitle: string,
  groupKey: string,
  items: ModuleSettingCatalogItem[],
  ...
  const rows = renderModuleSettingRowsHtml(items);
  const headerHtml = opts?.hideHeader
    ? ""
    : `<div class="flex items-baseline justify-between gap-3 border-b border-border px-4 py-3">
        <h3 class="text-sm font-semibold text-card-foreground">${escapeHtml(groupTitle)}</h3>
        <span class="shrink-0 text-xs tabular-nums text-muted-foreground">${items.length}</span>
      </div>`;
  return `
    <section
      id="${sectionId}"
...
function renderFohBrandMenuPageWithSettings(): string {
  const catalog = getModuleSettingsCatalog(FOH_BRAND_MENU_PATH);
  const items = catalog?.items ?? [];
  const brandsItems = items.filter((item) => item.groupKey === "brand-menu-presentation");
  const settingsItems = items
    .filter((item) => item.groupKey === "brand-menu-settings")
    .slice()
    .sort((a, b) => {
      // 品牌模式(531) → 品牌页作为首页(530)
      if (a.seq === 531 && b.seq === 530) return -1;
      if (a.seq === 530 && b.seq === 531) return 1;
      return a.seq - b.seq;
    });

  const brandsPanelHtml = renderFohBrandMenuGroupCard(
    "品牌管理",
    "brand-menu-presentation",
    brandsItems,
    { hideHeader: true },
  );
  const settingsPanelHtml = renderFohBrandMenuGroupCard("设置", "brand-menu-settings", settingsItems);
  ...
  return `
        <li class="list-none" data-module-setting-row-seq="${item.seq}">
          <div class="border-b border-border px-4 py-3">
            ...
            ${renderDefaultMainScreenPanelHtml(on)}
          </div>
        </li>`;
}
...
function renderStoreBrandSettingProductLinesSelector(
  ...
  return `
    <div class="mt-3 rounded-lg bg-muted/50 p-3" data-store-brand-setting-panel="${seq}">
      <div class="flex flex-wrap items-center gap-4">${boxes}</div>
    </div>`;
}
...
function renderModuleSettingStoreBrandSettingRow(item: ModuleSettingCatalogItem): string {
  ...
  return `
        <li class="list-none">
          <div class="border-b border-border px-4 py-3">
            ${renderModuleSettingTitleBlock(item)}
            ${renderStoreBrandSettingProductLinesSelector(item.seq, lines)}
          </div>
        </li>`;
}
...
function renderModuleSettingStoreBrandManagementRow(_item: ModuleSettingCatalogItem): string {
  return `
        <li class="list-none">
          <div class="px-4 py-3">
            ${renderStoreBrandManagementHtml()}
...
function runModuleSettingToggleSideEffects(seq: number, next: boolean): void {
      ...
      if (isPosOrderCartPosLinesSeq(seq)) {
        if (next) ensurePosOrderCartPosLinesDefault(seq);
...
function renderModuleHubSettingsPage(path: string, pageTitle: string): string {
  ...
  const isTeamSettingsHub = path === "/team/settings";
  const isFohBrandMenuSettings =
    path === "/operations/queue-call/brand-menu" ||
    path.startsWith("/operations/queue-call/brand-menu/");

  return `
    <div class="module-settings-main space-y-4">
      ${
        isStoreSettingsHub || isTeamSettingsHub || isFohBrandMenuSettings
          ? ""
...
function renderMain(): string {
  ...
  const isFohBrandMenu = isFohBrandMenuPath(path);
              ...
              : isAssetCenterMaterialsIframe
                ...
                : isFohMenuOrderLimitsIframe
                  ...
                  : isFohEmenuProIframe
                    ...
                    <div class="${tertiaryMainClass} min-h-0 overflow-hidden">${renderTeamReportsMainPanel(path, title, tabModule)}</div>
                  </div>`
                                        : isTipsManagementTertiary
                                          ? `<div class="${tertiaryRowClass}">
...
function renderDashboardPanel(path: string, title: string): string {
  ...
  const effectiveScopeDesc = describeEffectiveScope(resolveEffectiveScope(), getUiLocale());
  ...
  return `
    <div class="rounded-xl border border-border bg-card p-6 shadow-sm">
      ...
      <p class="mt-1 text-xs font-mono text-muted-foreground/80">${escapeHtml(effectiveScopeDesc)}</p>
...
function mount(): void {
  ...
  if (
    isLegacyBContentPath(authPath) ||
    (isLegacyBShellMode() &&
      !isEmenuLocalContentPath(authPath) &&
      !isKioskLocalContentPath(authPath) &&
      !isMPlatformContentPath(authPath) &&
      !isPitContentPath(authPath))
  ) {
    ...
    const normalizedPath = normalizeLegacyBPath(authPath);
    if (normalizedPath !== authPath) {
      replaceHashPath(normalizedPath);
      mount();
      return;
    ...
    bindLegacyBShell(mount);
    return;
  }

  if (
    isEmenuLocalContentPath(authPath) ||
    (isEmenuLocalShellMode() && !isKioskLocalContentPath(authPath) && !isMPlatformContentPath(authPath) && !isLegacyBContentPath(authPath) && !isPitContentPath(authPath))
  ) {
    if (isViewSwitchRestricted()) {
      exitEmenuLocalShell();
      replaceHashPath(APP_NAV_HOME_PATH);
  ...
  if (
    isKioskLocalContentPath(authPath) ||
    (isKioskLocalShellMode() && !isEmenuLocalContentPath(authPath) && !isMPlatformContentPath(authPath) && !isLegacyBContentPath(authPath) && !isPitContentPath(authPath))
  ) {
    if (isViewSwitchRestricted()) {
      exitKioskLocalShell();
      replaceHashPath(APP_NAV_HOME_PATH);
  ...
  if (isMPlatformContentPath(authPath) || (isMPlatformShellMode() && !isLegacyBContentPath(authPath) && !isPitContentPath(authPath))) {
