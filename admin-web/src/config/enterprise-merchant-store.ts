...
import { formatBid, generateNextBid, isBidFormat, normalizeBusinessId } from "./enterprise-merchant-bid";
import {
...
function notifyChainBrandOrgSyncForMerchant(merchantId: string): void {
  const merchant = readSnapshot().merchants.find((m) => m.merchantId === merchantId);
...
const GROUP_BRAND_FILLERS: Record<string, GroupBrandFillerSpec[]> = {
  ...
  "group-menusifu-cn": [
    { merchantId: "merchant-menusifu-cn-2", name: "MenuSifu 演示二店", code: "menusifu-cn-2", orgType: "single-store" },
...
function assignSeedMerchantBids(merchants: EnterpriseMerchant[]): number {
  ...
  for (const m of merchants) {
    if (m.bid) {
      const n = parseInt(normalizeBusinessId(m.bid).slice(1), 10);
...
function buildSeedSnapshot(): EnterpriseMerchantSnapshot {
  ...
  enrichLicenseFields({ enterprises, groups, merchants, brands, regions, stores, capabilities, changelog, requests, posStoreRequests, bidSeq, impersonationLogs: [], slaMetrics });

  return { enterprises, groups, merchants, brands, regions, stores, capabilities, changelog, requests, posStoreRequests, bidSeq, impersonationLogs: [], slaMetrics };
}
...
function migrateLegacyBidFields(snapshot: EnterpriseMerchantSnapshot): boolean {
  ...
  for (const merchant of snapshot.merchants) {
    ...
    if (legacyMid && !merchant.bid) {
      merchant.bid = normalizeBusinessId(legacyMid);
      delete (merchant as { mid?: string }).mid;
      changed = true;
    } else if (merchant.bid?.startsWith("M")) {
      merchant.bid = normalizeBusinessId(merchant.bid);
      changed = true;
    }
  }
  for (const store of snapshot.stores) {
    if (/^M\d{8}$/.test(store.storeId)) {
      store.storeId = normalizeBusinessId(store.storeId);
      changed = true;
    }
  }
  for (const request of snapshot.posStoreRequests) {
    ...
    if (legacyCreated && !request.createdBid) {
      request.createdBid = normalizeBusinessId(legacyCreated);
      changed = true;
    } else if (request.createdBid?.startsWith("M")) {
      request.createdBid = normalizeBusinessId(request.createdBid);
      changed = true;
    }
  }
  return changed;
...
function bootstrapLinkedStoreMerchant(
  ...
  const linked: EnterpriseMerchant = {
    ...
    code: slugCode(store.name) || merchantId,
    orgType: "single-store",
    status: "active",
...
function migrateOrgMidSchema(snapshot: EnterpriseMerchantSnapshot): boolean {
  ...
  for (const store of snapshot.stores) {
    if (store.linkedMerchantId && isBidFormat(store.storeId)) continue;
    ...
    if (orgMerchant.orgType === "single-store") {
      ...
      store.storeId = orgMerchant.bid!;
      store.name = orgMerchant.name;
      changed = true;
      continue;
    }
    if (!store.linkedMerchantId || !isBidFormat(store.storeId)) {
...
function needsOrgMidMigration(snapshot: EnterpriseMerchantSnapshot): boolean {
  ...
  if (snapshot.stores.some((s) => !s.linkedMerchantId || !isBidFormat(s.storeId))) return true;
  return false;
}
...
function resolveStoreLinkedMerchantId(
  ...
  if (isBidFormat(store.storeId)) {
    const byMid = snapshot.merchants.find((m) => m.bid === store.storeId);
    if (byMid) return byMid.merchantId;
  }
  const orgMerchant = snapshot.merchants.find((m) => m.merchantId === store.merchantId);
  if (orgMerchant?.orgType === "single-store") {
    if (orgMerchant.bid && (store.storeId === orgMerchant.bid || isBidFormat(store.storeId))) {
...
function normalizeOrgStoreLinks(snapshot: EnterpriseMerchantSnapshot): boolean {
  ...
  for (const store of snapshot.stores) {
    ...
    if (linkedMerchant?.bid && store.storeId !== linkedMerchant.bid) {
      store.storeId = linkedMerchant.bid;
      changed = true;
    }
    if (linkedMerchant && store.name !== linkedMerchant.name) {
...
function pickPreferredStore(a: MerchantOrgStore, b: MerchantOrgStore): MerchantOrgStore {
  const aMid = isBidFormat(a.storeId);
  const bMid = isBidFormat(b.storeId);
...
/** 同一组织下按挂载品牌 / 名称+地址去重，保留 BID 门店记录 */
...
export function getMerchants(filter: MerchantFilter = {}): EnterpriseMerchant[] {
  ...
  return snap.merchants.filter((m) => {
    ...
    if (filter.query) {
      ...
      const hay = [m.name, m.code, m.merchantId, m.primaryAdminEmail, m.contactName]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
...
export function mountMerchantToOrg(input: MountMerchantStoreInput, operatorEmail = DEMO_OPERATOR): MerchantOrgStore | null {
  ...
  appendChangelog(snap, {
    ...
    operatorEmail,
    detail: `挂载门店「${linked.name}」（BID ${linked.bid}）到组织`,
  });
  ...
  const merchant = createMerchant(
    {
      ...
      code: slugCode(request.storeName),
      groupId: ensureDefaultGroupForEnterprise(snap, request.enterpriseId).groupId,
      orgType: "single-store",
      timezone: "Asia/Shanghai",
...
export function getMerchantByBid(bid: string): EnterpriseMerchant | undefined {
  const normalized = normalizeBusinessId(bid);
  return readSnapshot().merchants.find((m) => m.bid && normalizeBusinessId(m.bid) === normalized);
}
