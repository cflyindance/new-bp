  ...
  return `<li class="relative" data-jme-tree-item="${encoded}">
    ...
    <div class="group relative flex min-h-16 items-start gap-2 rounded-2xl border px-3 py-2.5 transition focus-within:ring-2 focus-within:ring-blue-500/20 ${protectedNode ? "" : "cursor-grab active:cursor-grabbing"} ${cardTone}" draggable="${protectedNode ? "false" : "true"}" data-jme-drag-path="${encoded}" title="${protectedNode ? "兼容保留节点不可拖动" : "拖动可调整菜单顺序或层级"}">
      ...
      <div class="ml-1 flex max-w-[210px] shrink-0 flex-wrap items-start justify-end gap-1 self-start">
        ${issueBadges}<span class="flex h-6 shrink-0 items-center gap-1">${protectedNode ? `<span class="rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[9px] font-medium text-slate-500">兼容保留</span>` : ""}${node.display === false ? `<span class="rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[9px] font-medium text-slate-500">隐藏</span>` : ""}${node.disabled === true ? `<span class="rounded border border-rose-200 bg-rose-50 px-1.5 py-0.5 text-[9px] font-medium text-rose-600">已禁用</span>` : ""}</span><details class="relative" data-jme-menu-actions><summary class="grid h-6 w-6 cursor-pointer place-items-center rounded text-base leading-none text-slate-500 hover:bg-white hover:text-slate-900">…</summary><div class="absolute right-0 z-30 mt-1 w-48 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl"><button type="button" data-jme-open-add="${encoded}" class="block w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-slate-50">✎　新增子级</button><button type="button" data-jme-duplicate-menu="${encoded}" class="block w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-slate-50">▣　复制导航及子级</button><button type="button" data-jme-move-menu="${encoded}" class="block w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-slate-50">↕　指定移动</button><button type="button" data-jme-move-up="${encoded}" class="block w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-slate-50">↑　上移</button><button type="button" data-jme-move-down="${encoded}" class="block w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-slate-50">↓　下移</button><button type="button" data-jme-delete-menu="${encoded}" class="mt-1 block w-full rounded-lg bg-red-50 px-3 py-2 text-left text-xs text-red-500 hover:bg-red-100">▱　删除</button></div></details>
      </div>
    </div>
...
export function renderJsonMenuTree(
  ...
  return `<section class="flex min-h-0 w-[34%] min-w-[360px] max-w-[430px] shrink-0 flex-col border-r border-slate-100 bg-[#fafafa]" data-jme-tree-panel>
    <header class="shrink-0 border-b border-slate-200 px-6 pb-4 pt-6">
      ...
      <div class="mt-4 flex items-center gap-3 text-sm"><span class="font-medium text-slate-800">${visits.length} 项</span><span class="text-red-400">${errors} 错误</span><span class="text-amber-500">${warnings} 警告</span>${expandableCount ? `<button type="button" data-jme-toggle-expand-all class="ml-auto text-slate-500 hover:text-blue-600">${allExpanded ? "收起" : "展开"}</button>` : ""}</div>
    </header>
