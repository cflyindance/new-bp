...
/**
 * 自动下发：解析当前应写入的单店范围。
 * 无有效门店时返回 null（跳过本次自动下发）。
 */
export function resolveAutoDeploymentScope(): DeploymentScopeOption | null {
  const scope = readScopeFilters();
  const perspective = resolveChainDataPerspective();
  const snapshot = loadChainBrandOrgForContext();
  let storeId = scope.store || readChainAnchorStoreId() || "";

  if (!storeId) {
    const brandStoreIds = resolveCurrentBrandStores().map((s) => s.storeId);
    if (brandStoreIds.length === 1) {
      storeId = brandStoreIds[0] ?? "";
    } else if (perspective === "store") {
      storeId = brandStoreIds[0] ?? resolveCurrentAnchorStore().storeId;
    } else {
      storeId = resolveCurrentAnchorStore().storeId;
    }
  }

  if (!storeId) return null;

  const found = listAllMockStores().find((s) => s.storeId === storeId) ?? {
    storeId,
    storeName: demoStoreName(storeId),
  };
  const brandId =
    found.brandId ||
    readChainAnchorBrandId() ||
    scope.brand ||
    snapshot?.brands[0]?.merchantId;
  const brandName =
    snapshot?.brands.find((b) => b.merchantId === brandId)?.name ?? found.brandName;

  return {
    id: "current",
    label: `当前门店：${found.storeName}`,
    storeIds: [found.storeId],
    scopeLevel: "store",
    brandId,
    brandName,
  };
