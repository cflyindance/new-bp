      ...
      <table class="w-full min-w-[56rem] text-sm">
        <thead class="border-b border-border bg-muted/40 text-left text-xs text-muted-foreground">
          <tr>
            ...
            <th class="px-3 py-2.5 font-medium">状态</th>
            <th class="px-3 py-2.5 font-medium">门店数</th>
            <th class="px-3 py-2.5 font-medium">主管理员</th>
            <th class="px-3 py-2.5 font-medium">开通时间</th>
            <th class="px-3 py-2.5 font-medium w-20"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-border">
          ${merchants
            ...
            <tr class="hover:bg-muted/30">
              ...
              <td class="px-3 py-2.5 text-xs">${escapeHtml(getGroupName(m.groupId))}</td>
              ...
              <td class="px-3 py-2.5">${escapeHtml(getMerchantOrgTypeLabel(m.orgType))}</td>
              ...
              <td class="px-3 py-2.5 tabular-nums">${countStoresForMerchant(m.merchantId)}</td>
              <td class="px-3 py-2.5 text-xs">${escapeHtml(m.primaryAdminEmail ?? "—")}</td>
...
function renderListPage(): string {
  return `
    <div class="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden" data-enterprise-merchant-page="list">
      <div class="flex shrink-0 flex-wrap items-center justify-between gap-3">
        <p class="text-sm text-muted-foreground">检索与管理企业名下所有入驻品牌。</p>
        <a href="${merchantHref("/new")}" class="inline-flex h-9 items-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90">新建品牌</a>
      </div>
      <form class="flex flex-wrap items-end gap-3 rounded-xl border border-border bg-card p-4" data-enterprise-merchant-filter-form>
        ...
        <label class="flex min-w-[8rem] flex-col gap-1">
          ...
          <select name="orgType" data-merchant-filter="orgType" class="h-9 rounded-md border border-input bg-background px-2 text-sm">
            <option value="">全部</option>
            <option value="chain">连锁</option>
            <option value="single-store">单店</option>
          </select>
        </label>
        <label class="flex min-w-[12rem] flex-1 flex-col gap-1">
          <span class="text-xs text-muted-foreground">搜索</span>
          <input type="search" name="query" data-merchant-filter="query" placeholder="名称 / 编码 / 邮箱" class="h-9 rounded-md border border-input bg-background px-3 text-sm" />
        </label>
      </form>
      ...
      return card(`
        <div class="flex items-start justify-between gap-2">
          <div>
            ...
            <p class="text-xs text-muted-foreground">${escapeHtml(getMerchantOrgTypeLabel(m.orgType))} · ${storeCount} 门店</p>
          </div>
  ...
  return `
    <div class="space-y-4">
      ${card(`
        ...
        <p class="mt-1 text-xs text-muted-foreground">门店由本地 POS 申请通过后分配 BID（B00000000）；从企业下已入驻品牌中选择并挂载到本组织。</p>
...
export function bindEnterpriseMerchant(onMount: () => void): void {
  ...
  document.querySelector<HTMLFormElement>("[data-enterprise-merchant-create-form]")?.addEventListener("submit", (e) => {
    ...
    const input: CreateMerchantInput = {
      ...
      groupId,
      orgType: String(fd.get("orgType") ?? "single-store") as CreateMerchantInput["orgType"],
  ...
  document.querySelector<HTMLFormElement>("[data-merchant-request-form]")?.addEventListener("submit", (e) => {
    ...
    const input: SubmitProvisioningRequestInput = {
      ...
      orgType: String(fd.get("orgType") ?? "single-store") as SubmitProvisioningRequestInput["orgType"],
      contactName: String(fd.get("contactName") ?? ""),
      primaryAdminEmail: String(fd.get("primaryAdminEmail") ?? ""),
