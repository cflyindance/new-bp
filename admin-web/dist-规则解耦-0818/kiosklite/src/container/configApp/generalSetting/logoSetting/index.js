...
class LogoSetting extends Component {
  ...
  handleFileChange = (event, i) => {
    if (event.target.files[0]) {
      ...
      if (isImage(type)) {
        ...
        let path = this.state.kioskLogoList[i].type;
  ...
  handleDeleteImg = (type, i) => {
    let kioskLogoList = this.state.kioskLogoList;
    if (kioskLogoList[i].img) {
  ...
  render() {
    const { t } = this.props;
    const { isLoad, kioskLogoList, open, msg } = this.state;
    let imgPath = '';

    return (
      <div className={styles.picBox}>
        {isLoad
          ? kioskLogoList.map((item, idx) => {
              if (item.img) {
                imgPath = item.img;
              } else {
                if (item.type == 'logo') {
              ...
              return (
                <div className={styles.picItem} key={idx}>
                  <div className={styles.picItemBox}>
                    <div className={[styles.pic, styles.picLogo, styles['pic' + idx]].join(' ')}>
                      <img src={imgPath} />
                    </div>
                  ...
                  <div className={styles.logoPicTip}>
                    <div>
...
export default withTranslation()(LogoSetting);
