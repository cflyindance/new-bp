...
class PaymentType extends React.Component {
  ...
  getPaymentDisplayConfig = () => {
    ...
    const { canPayByCash, canPayByCard } = this.state;
    ...
    return {
      ...
      cashText,
      cashIconSrc,
      showBothPayments: canPayByCash && canPayByCard,
      showNoPaymentMessage: !canPayByCash && !canPayByCard,
    };
  };
  ...
  getPaymentStep = () => {
    ...
    return selfConfig?.configList?.find((each) => each.id === 24)?.value;
  };
  ...
  handleChoosePayByCard = async () => {
    ...
    if (selfConfig?.configMap?.id_5) {
      // 开通小费
      // isPayFirst === 1 先支付后小费
      const isPayFirst = this.getPaymentStep();
      if (isPayFirst) {
        // 是否满足信用卡支付
  ...
  async componentDidMount() {
    ...
    if (systemConfig?.KIOSK_PAYMENT_TYPE?.value) {
