...
const PromotionDealList = (props) => {
  ...
  const localPromotionDrawerProps = useMemo(() => {
    const displayName = Array.isArray(localPromotionDrawerState.info?.text)
      ? localPromotionDrawerState.info?.text?.[0]
  ...
  const kioskBuyGiftList = useMemo(() => {
    return buyGiftRule?.map((giftInfo) => {
      ...
      return {
        text: t('buy-x-give-n', { buyNumber, giftsNumber }),
        commercialType: 'all',
        isLocalPromotion: true,
    ...
    const fetchPromotionList = async () => {
      ...
      try {
        ...
        const mappedList = filteredList?.map((each) => {
          ...
          return {
            text: getPromotionCenterActivityRuleText({
              t,
              activityRule,
              type,
              promotionName,
