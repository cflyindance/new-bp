...
主动选择微应用或编辑显式微应用节点时，在“层级与页面”下展示独立配置区。仅继承父级微应用且未主动覆盖的节点不展示必填配置区，只显示继承来源：
...
- 后代存在显式覆盖时，父节点仍可修改非结构信息。
- 如果把显式微应用祖先切换成其他用途会让后代失去唯一的微应用继承来源，则硬性阻止切换，并列出依赖该祖先的后代；用户必须先把这些后代改成显式配置或其他用途。
- 如果移动一个依赖外部祖先微应用配置的继承节点或子树会使其失去继承来源，则硬性阻止移动；移动包含自身显式微应用根的完整子树允许执行。
- 删除节点及其完整子树不会产生孤立继承节点，继续使用现有删除确认，不因微应用继承额外阻止。
...
- 禁用菜单在商家后台普通预览和全屏预览中显示，但选择、页面打开和子级进入动作无效；在编辑器菜单树中仍可选择、展开、编辑和重新启用。
