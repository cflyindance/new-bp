...
| 版本 | 日期 | 修改内容 |
| --- | --- | --- |
| V1.1 | 2026-09-23 | 取消单员工重复明细选择，同步动态周数、PDF 反馈、邮箱行为及当前打印入口边界 |
| V1.0 | 2026-09-03 | 确认详细/精简明细、A4 打印与 PDF/CSV/邮箱导出 |

## 目标

员工详情同时支持“详细明细”和“精简明细”。详细明细保留现有内容与结构；精简明细参照纸质 Payroll Report，在 A4 纵向纸张中集中展示身份、周期、薪资汇总、当前期全部考勤、声明与签名。打印与导出允许选择明细版本，并支持可记忆的“强制一页 / 按内容分页”策略。

## 已确认决策

2026-09-23 同步：以下以最后确认的交互为准；单员工导出取消重复选择明细，批量导出仍保留独立明细选择。灵活周期的动态周数及实现边界关联 `2026-09-22-payroll-period-employee-display-design.md` 和 `2026-09-22-flexible-payroll-period-schedule-design.md` 第 10 节。

- 员工详情标题栏使用“详细明细 / 精简明细”两个页签。
- 当前打开员工详情默认详细明细；点击页签切换详细/精简，不修改核算数据。当前实现每次打开重置为详细，不承诺跨关闭记忆页签。
- 打印分页设置提供“强制一页”和“按内容分页”，跨会话记住最后选择。员工详情底栏「打印方式」在详细明细与精简明细中均仅隐藏，不删除控件与 localStorage / 打印导出逻辑。
- 打印按钮打印当前页签对应版本。
- 单员工“导出结果”直接选择 PDF、发送到邮箱或 CSV，不再出现“选择明细”步骤。
- 导出类型跟随当前详细/精简页签；开始操作时固定当前员工、期次和明细版本，避免异步过程中串用其他数据。
- 详细与精简 PDF 均显示生成中、成功或失败状态；失败可重试，不允许点击后无反馈。邮箱页面取消关闭且不提交，保存/发送执行相应动作并反馈结果；真实邮件送达须有服务端证据，不能以演示反馈代替。
...
- 当前统计期相交的全部工作周日期范围；双周通常两组，半月/月度/自定义动态追加，不能固定只保留 Week 1、Week 2。
...
点击“导出结果”直接展示 PDF、发送到邮箱、CSV 三种方式；明细版本取当前页签，不再二次选择明细。
...
| 需要更换明细版本 | 关闭菜单并切换页面页签，再打开导出；菜单不提供重复版本选择 |
...
- 员工详情可在两个页签间切换，详细明细与当前实现一致。精简明细以用户提供的纸质 Payroll Report 为视觉方向，但验收以本文档为准：A4 使用 8mm 页边距；身份/周期头 10–12pt，表格 7–9pt，声明 7–8pt；字段顺序严格为“身份与周期 → 汇总 → 本期全部工作周 → 声明 → 签名 → 门店”。
...
- 页面两种明细分别支持 PDF/CSV/邮箱，导出菜单不重复选择明细，详细 PDF 与精简 PDF 同样反馈进度和结果。
