...
以下入口标题旁展示小标签「云产品」（英：Cloud）：

一级「云产品」：商品中心、外卖/来取、促销中心、会员中心、评价中心、预约等位中心、报表中心、礼品卡中心。

一级「部分-云产品」：营销中心、打印中心、消息中心。

一级「非MVP版本」：门店管理（集团/品牌视角）、库存管理中心、硬件管理中心、素材中心、日志管理。

视角切换「非MVP版本」：集团总部、M 平台。

二级「非MVP版本」：团队管理 · 绩效与培训、团队管理 · 设置；系统设置除「下发记录」外全部二级（区域与显示、数据与备份、连接与服务、高级与诊断、平台预设）。

二级「云产品」：促销活动、打印装修、营销活动、手动营销、屏保、消息模板 / 消息配置 / 消息额度、小费管理、薪资管理、员工报表。

实现：`cloudProductBadgeForNavModule` / `cloudProductBadgeForNavChild`；全量标签天空色，部分标签琥珀色，非 MVP 标签玫红色。门店管理与团队上述两项不再画红框。
