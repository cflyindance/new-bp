...
| 存量数据处理 | 自动拆分多关联（稳定派生 id）+ 按营业时间合并为一条，汇总全部日期并去重升序 |
| 合并冲突取值 | 采用最早一条旧规则（按 `dates[0]` 升序，其次 `name`、非派生 id 优先、再 `id`）的 `name` / `openTime` / `closeTime` / `mode` / `activeDays` |
...
1. `raw.dates` 为数组 **且** 过滤合法日历日期后 `length ≥ 1` → 采用该结果，再截断至最多 366 项（取升序后前 366 个）
2. 否则（含 `dates: []`、全非法、字段缺失）回退 `raw.fromDate` ~ `raw.toDate` → 按天展开为闭区间内所有日期
3. 否则 legacy `raw.date` → 单元素数组
4. 全部缺失 → `[currentDate()]`

展开上限 366 天，**自 `fromDate` 起向前取最多 366 天**（含起点）。超长脏数据（如 `2000-01-01 ~ 2099-12-31`）截断为 `2000-01-01` 起连续 366 天，优先保证不卡死；不保证保留「今天附近」。`raw.dates` 直接超长时同样截到 366。结果统一去重升序。

「合法日期」判定：在现有 `isIsoDate` 格式校验（`YYYY-MM-DD`）之上，增加真实日历校验（用 `Date.UTC` 回读年月日，三者与输入一致），拒绝 `2026-02-31` 等假日期。迁移与保存共用该判定。
...
2. **第一条保留原 `id`**；其余克隆使用**稳定派生 id**：`bhx-split-${原id}-${scheduleId}`（不调用 `newId`，保证同一份存储被多次只读读取时 id 不变，卡片上的 `data-exception-id` 可命中编辑/删除）
3. 若派生 id 已与**全量记录 id**（含尚未处理的原始记录、已产出的拆分结果）冲突，后缀递增 `-2`、`-3`… 直至唯一（同样确定性）
4. 每条克隆的 `scheduleIds` 改写为 `[该 scheduleId]`；其余字段（含完整 `dates`）原样复制
5. 原多关联记录从结果中移除，替换为上述独立记录

**步骤 B — 按营业时间合并**

1. 孤儿（`scheduleIds` 为空）不参与合并，原样保留
2. 对其余记录按唯一的 `scheduleId` 分组
3. 同组多条：按 `dates[0]` 升序、其次 `name`、再「非派生 id 优先」（不以 `bhx-split-` 开头优先）、再 `id` 排序，取首条为基准
4. 基准条的 `dates` 并入同组其余条的全部日期，去重升序（仍截到最多 366）
...
2. **生效时间列表**（新增，替换原单日期 / 日期区间输入）
   ...
   - 底部「+ 添加生效时间」追加新行，默认值为今天；已有 366 行时禁用该按钮
   - 仅剩 1 行时隐藏该行的删除按钮
...
- **落盘形态写死为拆条**：每个合法 `scheduleId` 各写一条独立记录，`scheduleIds: [id]`
  ...
  - **编辑（已关联）**：锁定为当前条已有的唯一 `scheduleId`，禁止改选；只更新当前这条
  - **编辑（孤儿，`scheduleIds` 为空）**：允许重新单选一条营业时间（不做多选）；保存时写入 `scheduleIds: [所选 id]`，并走单规则占用校验。若未选 → `请至少选择一条营业时间`
...
1. 名称（非 MVP）/ MVP 默认名
2. dates：至少一个合法日期；非法 → `请选择有效的生效日期`；重复 → `生效日期不可重复`；全空 → `请选择生效日期`（exclude 时「不生效日期」）；超过 366 个 → `生效日期最多 366 个`
3. 星期、开闭市（沿用现状）
4. `scheduleIds` 非空
...
- 已关联记录编辑态改挂其他营业时间（锁定当前 `scheduleId`；孤儿除外，见 §5.2）
...
11. 迁移：存量一条 `scheduleIds: [A,B]` → 读后拆成两条（一条保留原 id，一条稳定派生 id），分别挂在 A/B 卡下；连续两次只读 id 不变；分别编辑/删除均可命中，互不抹掉对方关联
...
18. 孤儿重新关联：从 583 编辑孤儿，单选一条尚无额外时间的营业时间 → 保存成功；若该营业时间已有规则 → 拦截
19. 假日期：写入 `fromDate=2026-02-31` 或表单填入该值 → 规范化不采纳，保存时报非法日期
20. 拆分后再合并：存量同时有 `scheduleIds: [A,B]` 的多关联记录 + 一条已挂在 A 上的独立记录 → 读后 A 下只剩一条，日期为并集；B 下有拆分出的一条
21. dates 上限：弹窗已有 366 行时「+ 添加生效时间」禁用；保存超过 366 时报错
