...
- 打开某个 Hub 的搜索态时创建会话；清洁状态下可直接接受新关键词。
- 有草稿时更换关键词，只有“保存并继续”或“放弃修改”成功完成后，才能接受新关键词；两种结果都会使旧关键词下的草稿归零。
- 选择“取消”时不接受新关键词，原关键词和草稿保持不变。
- 保存成功后会话继续存在，但回到清洁状态。
- 关闭滑层或切换 Hub 时，在离开保护完成后销毁原会话。
- 已持久化但待下发的记录不属于可放弃草稿，不触发“是否保存”对话框；它由独立下发重试队列继续管理。
...
同一设置在当前已接受关键词的结果重渲染、分批挂载或滚动离屏后再次出现时，读取同一份会话草稿。关键词变化一旦被接受，旧关键词下的草稿已经保存或放弃，不跨已接受的关键词变化保留。会话草稿保存在内存中，不跨页面刷新恢复。
...
统一保存表示一次用户确认和一次编排流程，不虚构跨配置域的底层原子事务。每个来源必须维护明确的阶段状态：

```ts
type SearchSaveSourceState =
  | "draft"
  | "validated"
  | "persisted-awaiting-deploy"
  | "deployed";
```

- 校验或持久化失败的来源停留在 `draft`，重试时重新执行校验和持久化。
- 持久化及变更记录成功、下发失败的来源进入 `persisted-awaiting-deploy`；其待下发快照、变更记录引用和幂等键立即移交独立下发重试队列，搜索会话中的草稿随即清除。重试只重新下发，不重复持久化或生成变更记录。
- 下发成功的来源进入 `deployed` 并从待处理队列移除。
- 每次统一保存生成稳定的 `saveOperationId`，来源批次使用 `saveOperationId + settingsPath + domainKey` 作为幂等键。

“保存并继续”以持久化成功为保存成功边界。若配置已持久化但即时下发失败，系统显示“设置已保存，等待下发”，把记录交给重试队列，并继续执行用户原先的搜索、导航或关闭操作。“放弃修改”只适用于仍处于 `draft` 的内容；一旦持久化成功，不再提供回滚式放弃。
...
4. 读取顺序为“当前搜索会话草稿 → 持久层值 → 规范默认值”；待下发阶段已经持久化，因此直接读取持久层值，并另行展示下发状态。
...
- 校验或持久化失败时保留失败项草稿、关键词和结果页面。
- 持久化成功但下发失败时，设置已经保存；把 `persisted-awaiting-deploy` 快照、已生成的变更记录引用和稳定幂等键移交独立下发重试队列，搜索会话不再保留可放弃草稿。
- 错误信息按来源设置页或配置域展示。
- 用户可从结果提示或全局下发状态直接重试，无需重新输入；重试只执行下发。
- 保存栏的“待保存 N 项”才计入搜索会话脏状态并触发离开保护；“待下发 M 项”作为独立状态提示，不阻止修改关键词、导航、关闭滑层或切换 Hub。
- 已成功来源不重复提交；幂等键保证同一来源重试不会重复持久化、记录或下发。
...
- 搜索会话草稿读写与修改数量。
- 保存、放弃、取消及待执行操作状态转换。
- 有草稿时新关键词的保存、放弃、取消三种接受规则。
- 来源从 `draft` 到 `persisted-awaiting-deploy` 再到 `deployed` 的阶段转换、搜索会话到下发重试队列的移交及幂等重试。
...
- 取消后恢复关键词、草稿与滚动位置。
- 校验失败时整批不提交并定位错误。
- 部分下发失败后只在独立重试队列保留失败来源的 `persisted-awaiting-deploy` 阶段；搜索会话可正常离开，并验证重试不会重复持久化或生成变更记录。
- 保存成功后保留关键词和结果页面。
...
8. 持久化失败后草稿不丢失；下发失败后设置仍视为已保存，待下发阶段移交独立重试队列，可直接重试且不重复持久化或记录。
...
- 所有搜索滑层均使用同一套设置编辑表面。
- 所有规范设置结果都必须使用公共设置编辑表面完整渲染；无编辑权限时完整只读展示，不允许以“不可内嵌”为由降级成只跳转定位。
