...
打开编辑器时先读取服务端共享草稿：存在草稿时恢复草稿并显示“未发布草稿”提示；不存在草稿时读取当前已发布配置作为编辑基线。保存草稿不影响当前生效菜单。新建空白配置与编辑当前配置是两个明确入口。本期不提供文件导入。“放弃修改”删除共享草稿并重新加载当前发布配置。

新建空白配置时初始化全部必需根字段：

- `_id`：新建时由客户端生成 24 位十六进制字符串，服务端必须原样保存，不再替换。这样新文档在首次保存前也具有可导出的稳定 ID。
...
- `readDraft(): Promise<MenuDocument | null>`：读取共享草稿；没有草稿时返回 `null`。
- `saveDraft(document: MenuDocument): Promise<MenuDocument>`：保存共享草稿，不改变线上菜单，并返回服务端最终保存的业务文档。服务端不得替换 `_id`。
- `deleteDraft(): Promise<void>`：放弃共享草稿。
- `publish(document: MenuDocument): Promise<MenuDocument>`：原子替换当前生效文档并返回服务端写入最终 `updatedBy`后的文档。

发布成功后服务端原子删除对应共享草稿。接口请求和响应中的业务文档均使用第 3 节的 `MenuDocument`，不得向文档增加版本号或发布状态。并发控制、草稿所有者和审计信息属于 HTTP 标头或服务端存储元数据，不进入业务 JSON。导出由浏览器直接下载当前序列化结果，不依赖服务端。
...
- 序列化白名单测试递归断言根、节点、`i18nInfo`、`microAppConfig`、`accessControl`和`permission`均未出现结构外业务字段。
...
- 发布失败或离开提醒不会丢失编辑内容。
- 保存共享草稿后刷新页面或由其他授权用户打开编辑器，能够恢复同一草稿；放弃或发布成功后不再恢复旧草稿。
- 新建文档的 24 位十六进制 `_id`在保存草稿、发布和导出过程中保持不变。
