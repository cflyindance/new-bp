...
| 商品范围 | 如 `指定菜品 12 个`、`菜品集 8 个商品`、`分类 3 个`；无范围显示 `—` |
...
- 每家门店独立统计，不跨门店相加或去重。
- 菜品按 `productLineId + dishId` 去重；分类按 `productLineId + categoryId` 去重；菜品集商品按 `productLineId + dishId` 去重。
- 单门店显示 `<对象名称> N 个`，例如“指定菜品 12 个”或“菜品集 8 个商品”。
