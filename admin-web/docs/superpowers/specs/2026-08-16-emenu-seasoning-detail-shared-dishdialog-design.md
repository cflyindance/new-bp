...
- 菜单卡片：有启用调味关联时显示 Detail；无关联不显示。
- 共用 `DishDialog`，以 `entrySource: 'detail' | 'add'` 区分入口。
- Detail 入口：在 OptionList 下方挂载 `SeasoningBlock`；加购时写入调味选择快照。
- 加号入口：不挂载 `SeasoningBlock`；购物车行不带调味。
- 无规格 / 无 POS Option 时，Detail 仍打开共用详情（右栏可仅有调味区），确认后加购；调味非必选。
- 选择互斥与订单快照语义对齐既有调味食客端规则（`seasoning-terminal-rules` / `OrderSeasoningSelection`）。
...
菜单卡片
  ...
  │                 └─ 加入购物车 → 菜品行 + OrderSeasoningSelection[]
...
| 单元 | 职责 |
| --- | --- |
| 菜单卡片（DishItemCard 等） | 根据调味快照判断 Detail 显隐；分别触发 detail / add 打开详情 |
| DishDialog | 共用容器；按 `entrySource` 决定是否渲染 SeasoningBlock |
| SeasoningBlock | 渲染关联候选项、互斥选择、可选 priceDelta 展示 |
| 调味快照读取 | 终端本地完整快照（version + relations + options）；失败沿用既有缓存策略 |
| 购物车行扩展 | 仅 detail 路径附加调味选择快照 |

## 4. 入口与展示规则

1. **Detail 显隐**：商品在当前有效调味快照中存在**启用**关联 → 显示 Detail；否则不显示。
...
- 右栏自上而下：
  ...
  4. 备注等既有区块保持原顺序约定（若存在，调味块紧接 OptionList 之后、备注之前；实现时与现有 RightPanel 结构对齐）
...
- 从门店调味快照按 `productId` 过滤启用关联，结合 Option 库（停用 Option 隐藏）。
- 动作展示顺序固定：添加、少放、多放、不要（与后台一致）。
- 同动作内顺序使用关联 `sortOrder`。

### 选择规则

- 同一 Option 的多个动作互斥（后选替换先选）。
- 不同 Option 可同时选择。
- 调味**非必选**：可不选任何调味直接加购。

### 购物车

- `entrySource=detail`：菜品行附带 `OrderSeasoningSelection[]`（至少含 `action`、`optionId`、`priceDelta`；名称/编码等快照字段按既有 2026-08-12 规格补齐，避免改名后历史不可解释）。
- `entrySource=add`：不读写调味；行上无调味或空数组。
- 成交价：在原有菜品价基础上累加所选 `priceDelta`（实现时与现有价格计算钩子对齐）。
...
| 打开 Detail 时关联已被快照更新删空 | 打开详情但不渲染调味区（或提示后等同无调味）；不阻断规格选择与加购 |
| Option 停用 | 候选项不出现；已在购物车的历史行保留快照 |
| 仅有调味、无规格 | Detail → 确认可加购 |
| 同时有规格与调味 | Detail 一次完成规格 + 调味 + 加购 |

## 8. 验收标准

1. 有启用关联的商品菜单上可见 Detail；无关联不可见。
2. 点 Detail：详情中调味区在 Option 下方；加购后购物车行含调味选择（若有选）。
3. 点加号进入详情：不可见调味区；加购行无调味。
4. 同 Option 多动作互斥仍成立。
