...
- 规则列表对草稿和启用规则统一显示「编辑」，文案、按钮层级和进入编辑器的行为不做区分；草稿状态本身仍通过状态标签表达。
...
- 规则列表不提供规则关键词筛选，保留五类核心筛选；桌面端从左到右依次为：门店、状态、人数、轮次、时间，最右侧为“重置筛选”；移动端按相同顺序换行。
- 人数与时间选项根据现有规则动态生成。
- 轮次支持每轮、多轮、与轮次无关；状态支持启用、禁用、草稿；门店仅展示现有规则涉及的门店。
- 所有条件组合生效；重置筛选恢复完整列表。
- 数量角标显示“筛选结果数 / 总数”。新增、编辑、复制、删除和状态切换后保留当前筛选并重新计算结果。

### 10.5 现有规则字段设置

- 规则列表按六步配置模型建立字段注册表，步骤 6 仅复用前五步汇总，不新增业务字段。
- 固定字段为规则名称、状态、操作，不允许隐藏。
- 首次进入采用平衡模式，依次显示：规则名称、规则策略、人数场景、商品范围、生效门店、生效时间、超限授权、状态、操作。
- “规则策略”合并展示限购主体、轮次规则、限购对象；三个原子字段仍可在字段设置中单独勾选。
- 更多字段按步骤分组：规则描述、儿童人数口径、人数/轮次区间、参与门店、产线、目标商品数、数量完成度、授权范围/权限、日期/周期/时段、会员范围等。
- 用户只控制显示或隐藏，列顺序由系统按重要度固定，不支持拖拽。
- 字段偏好使用独立、带版本号的 localStorage 键按浏览器保存；不写入规则、草稿或发布快照。非法旧字段自动忽略，固定字段自动补回。
- 新规则优先读取 `editorDraft/authoringDraft`；历史规则回退到 `type/round/method/persons/timeLabel/storeLineLabel` 等扁平字段，缺失值显示 `—`。
