...
- 适用门店。
- 适用产线或点单渠道，写入既有 `productLines` 字段；不新增独立 `terminalTypes` 规则条件。
- 适用菜单，写入 `menuIds`。
- 适用订单类型，写入 `orderTypes`。

页面中的 POS、eMenu、SDI 等名称属于产线或点单渠道的展示名称。确认页的「影响终端」由 `productLines` 和实际下发结果推导，不是另一套独立命中条件。
...
- 「按分类」时选择器叶子节点停在分类。
- 「按菜品」时选择器叶子节点停在菜品。
- 主页面展示已选分类或菜品数量，并可重新打开选择抽屉。
- 商品选择使用 560–720px 抽屉，底部固定「取消 / 确定」。未点击确定的临时选择不写入主表单。
...
`productLineId` 必须属于规则第二步的 `productLines`。服务端判定时先匹配当前产线，再读取同一 `targetId` 的数量；冲突检测增加产线交集维度。旧数据迁移时，如果旧规则只有一条数量值，则为该规则每个适用产线复制一条相同值并导入为草稿，用户复核后才能发布。
...
门店、产线、菜单和订单类型已在第二步定义。最后的下发流程再次选择实际发布门店，实际发布门店必须是第二步适用门店的子集。
...
- 分类或菜品目标；
- 人数与轮次数量矩阵；
- 门店、产线、菜单、订单类型、日期、星期、时段、会员范围和儿童计数策略；
- 超限授权范围、默认范围、权限和原因要求；
- 影响的门店和终端；
- 与其他规则叠加后的最小有效额度提示。
...
- 门店、产线、菜单、订单类型、时间、星期、会员和儿童计数策略完整。
- 授权默认范围属于已启用范围。
- 每个已启用授权范围存在有效权限。
- 商品、分类和角色仍然存在。
- 同型规则不存在禁止发布的范围冲突。
...
10. 七个步骤的条件展示符合本方案，第二步包含门店、产线、菜单和订单类型，第四步包含场景条、产线 Tab、批量设置、复制和完成度提示。
