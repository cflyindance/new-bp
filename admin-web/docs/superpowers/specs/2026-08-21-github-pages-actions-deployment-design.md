...
将 GitHub Pages 发布产物限定为管理端构建目录，避免 Pages 的分支式 Jekyll 构建扫描仓库根目录并失败，同时保持既有 URL 路径兼容。
...
3. 上传前创建仅含 `admin-web/dist` 的临时 artifact 目录，保留 `/admin-web/dist/index.html` 既有公开 URL；
4. 使用最小权限：构建任务读取仓库并写入 Pages，部署任务只使用 Pages 与 OIDC 权限；
5. 通过并发组取消过期部署，确保最新提交最终上线。

## 验收

- 新工作流的 YAML 可解析，artifact 只包含 `admin-web/dist`；
