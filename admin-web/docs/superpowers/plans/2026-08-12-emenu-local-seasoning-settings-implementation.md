...
1. 使用 `/api/v1/emenu-local/seasoning` 前缀实现以下端点：
   ...
   - `POST /relations/preview`：根据商品选择 token 和多组 `actionOptions[]` 创建预览草稿；
...
5. 第二步合并动作与 Option 配置：默认不展示动作；“新增动作”弹窗可批量选择固定动作。左侧动作列表切换当前动作，右侧“添加 Option”弹窗多选启用 Option，并逐项维护该 `(动作, Option)` 的批次默认价格。
6. 各动作的 Option 和价格独立保存；所有已添加动作均至少配置一个 Option 后才允许预览。商品选择变化清空全部动作配置和预览；任一动作、Option 或价格变化保留商品及其他动作配置，但删除旧预览草稿后重建。
7. 第三步以 `actionOptions[]` 创建多动作 `previewToken`，按状态汇总和游标页显示候选。移除候选、覆盖单条 `(商品, 动作, Option)` 价格、价格差异决策和停用关系决策均写入服务端预览草稿；筛选或翻页后保持状态。
8. 使用服务端返回的全量未解决数量控制提交。提交只传 `previewToken` 和 `expectedVersion`；成功后展示新增、更新、重新启用和跳过数量，刷新总览并关闭向导。
9. `previewToken` 失效时返回“设置动作与 Option”步骤并重新生成；若商品选择 token 同时失效则返回商品步骤。普通关闭和路由离开执行脏状态保护；用户确认放弃或保存成功后显式删除选择与预览草稿。
...
5. 启动 `npm run dev:emenu-local`，完成浏览器主路径：
   ...
   - 按“商品 → 动作与 Option → 预览”完成批量关联，验证批量新增动作、每个动作独立多选 Option 和定价，以及预览分页、跨页冲突决策和未解决总数；
