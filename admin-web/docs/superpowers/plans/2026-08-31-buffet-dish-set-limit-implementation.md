...
- 每店独立保存菜品集商品，至少两个有效商品才能发布或启用。
- 数量矩阵按“门店 × 人数区间 × 轮次区间”保存一个共享额度，不包含产线或单品维度。
- 同口径、同条件菜品集商品重叠时发布/启用失败；与单品和分类规则允许叠加。
- 运行时跨产线聚合集合商品数量并进行原子校验。
...
- v1 菜品/分类规则加载结果不变。
- v2 菜品集按 `productLineId + dishId` 去重商品。
...
- 单删、批删、全删后商品和失效数量同步更新。
- 少于两个商品允许保存草稿，但步骤状态显示待完善。
...
- 删除商品时清理相关选择状态，但不得按商品删除共享额度；商品少于两个时把发布完整性标记为失败。
...
- `targetEntries` 为菜品集输出商品身份列表，而不是一个集合伪 target。
- `sameTarget` 对菜品集按 `storeId + lineId + dishId` 比较。
- `findConflict` 仅在双方均为 `dish_set` 且 mouth 相同的情况下执行商品交集阻断；跨对象类型不阻断。
- 增加发布完整性错误码和可诊断详情。

修改 `order-limit-flow.js`：

- `validateStep`、发布前校验和启用校验接入最小商品、有效性、矩阵完整性与重叠冲突。
- 保存草稿保留非阻塞提示。
...
- 发布快照保留每店商品身份与共享矩阵。
- 增加菜品集商品匹配和 `quantityByRule` 聚合入口，使一条规则的跨产线商品汇总为单个 `Q`。
...
- 复制产生独立 draftId，保留各店商品和共享矩阵，并重新校验冲突。
- 编辑旧 v1 规则不自动升级。
...
2. 打开“前厅管理中心 → 自助餐规则”。
3. 新增规则并选择“按菜品集限购”。
4. A 门店跨两条以上产线选择至少两个菜品；B 门店选择不同商品。
5. 查看已选商品筛选、单删二次确认、批删和全部删除。
6. 进入限购数量，确认每店每场景只有一个共享额度输入，并可查看商品。
7. 验证 0、空值和正整数状态。
