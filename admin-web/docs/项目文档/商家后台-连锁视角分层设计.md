...
| **品牌多门店** | `brand` | 当前选中**品牌**下的全部门店（多品牌账号可在授权范围内切换品牌） | 授权 **1 个品牌** → 顶栏品牌**只读锁定**；授权 **≥2 个品牌** → 顶栏品牌**下拉切换**；区域 / 门店可选（MVP 下隐藏区域筛选） |
...
|------|------|------|------|
| 集团总部 | 下拉（含「全部」） | 下拉 | 下拉 |
| 品牌多门店 | 授权 1 品牌：**只读标签**；授权 ≥2 品牌：**下拉切换** | 下拉（MVP 隐藏） | **页内选择**（配置页）；顶栏不展示门店下拉 |

进入二/三级**门店配置**导航（如各模块「设置」、硬件设备、前厅按产线设置）时，页顶展示**配置门店**选择条；未选门店时仅显示引导占位，选中后才渲染设置表单。汇总类页面（Dashboard、权限、品牌列表等）不受此规则约束。
| 门店 | **只读**（当前门店名） | 隐藏 | 隐藏 |
...
| 演示账号 | 默认布局 | 默认数据视角 | storeAccess | MVP 品牌多门店 · 顶栏品牌 |
|----------|----------|--------------|-------------|---------------------------|
| `zhangji.admin@menusifu.cn` | 连锁版 | 集团总部（MVP 降为品牌多门店） | `all` | 下拉（集团内全部品牌） |
| `hq.admin@menusifu.cn` | 连锁版 | 集团总部（MVP 降为品牌多门店） | `all` | 下拉（集团内全部品牌） |
| `brand.ops.east@menusifu.cn` | 连锁版 | 品牌多门店 | 张记火锅 + 张记串串 | 下拉（2 品牌可切换） |
| `brand.ops.single@menusifu.cn` | 连锁版 | 品牌多门店 | 仅张记火锅 | 只读锁定 |
| `xiaoming.wang@menusifu.cn` | 门店版 | 门店 | 上海陆家嘴店 | — |
| `cashier.li@menusifu.cn` / `floor.zhang@menusifu.cn` | 门店版 | 门店 | 上海陆家嘴店 | — |
...
当顶栏**版本切换**选择 **MVP**、**视角切换**选择 **品牌多门店**时：

| 能力 | MVP 行为 |
|------|----------|
| 视角入口 | 隐藏「集团总部」「M 平台」；连锁版默认进入品牌多门店视角 |
| 顶栏品牌 | **支持切换**（当账号 `storeAccess` 授权 ≥2 个品牌时展示下拉；仅 1 个品牌时只读锁定） |
| 顶栏区域 | 隐藏整块区域筛选 |
| 顶栏门店 | 展示下拉，选项随当前选中品牌联动刷新 |
| 顶栏门店（配置页） | **隐藏**；进入二/三级配置导航后，在**页内**先选门店再编辑设置 |

**团队管理**（品牌多门店视角下均需页内选门店）：休息与加班、设置（`/team/settings`）。**角色与员工**的门店筛选位于 TipOut「员工列表」标题下方；**员工打卡**的门店筛选位于各 Tab 工具栏（日期左侧 / 规则设置顶部）；**排班**的门店筛选位于日期选择器左侧，均不走外层配置门店条。

**前厅管理中心**（品牌多门店视角下均需页内选门店）：餐位平面图、品牌与菜单、菜单下单限制、品类设置、分类设置、eMenu Pro。

**财务中心**（品牌多门店视角）：收银记录与审计（付款记录、钱箱登入退出记录）在时间选择器左侧精简选门店，不走外层配置门店条。

**营销中心**（品牌多门店视角）：广告列表在名称搜索左侧精简选门店；广告数据按门店隔离存储。
| 侧栏导航 | 隐藏「品牌门店列表」等未来版本模块 |

### 典型业务场景

**一个商家账号管理多个品牌，每个品牌下又有多个门店**（例如集团授权品牌 A、B 给同一运营账号）：

1. 登录后选择 **MVP 版本** + **连锁版 · 品牌多门店**。
2. 顶栏品牌下拉列出授权范围内的品牌（如张记火锅、茶语一家），**不可见未授权品牌**。
3. 切换品牌后，区域 / 门店选项与报表、订单、商品等页数据范围**同步刷新**至该品牌下门店。
4. 若账号仅授权单一品牌，顶栏品牌显示为只读标签，行为与单品牌经理一致。

**有效数据范围**（与 §4 一致）：

```
有效可见范围 = 品牌多门店视角上限 ∩ 员工 storeAccess ∩ 顶栏品牌/门店细筛
```

演示账号详见 [演示账号-连锁视角联调用例](./演示账号-连锁视角联调用例.md)。典型：`brand.ops.east@menusifu.cn`（张记火锅 + 张记串串，顶栏可切换）；`brand.ops.single@menusifu.cn`（仅张记火锅，品牌只读锁定）。
