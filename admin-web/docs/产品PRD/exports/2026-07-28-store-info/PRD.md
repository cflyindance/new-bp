...
| SI-14 | 操作 | 额外时间：独立设置项 seq 583；亦可在 418 卡片下添加；一条规则可含多个单日生效日期（最多 366）；每条营业时间下最多 1 条额外时间；include/exclude；新数据 `scheduleIds` 长度 1；删规则摘引用，空关联进待补全；旧区间/多关联读取时稳定拆分合并 | → SI-11 |
...
**用户故事**：作为店长，我希望为某条营业时间配置一份额外时间，并在多个单日生效。

- Given 某营业时间尚无额外时间 When 点「+ 添加额外时间」并添加多个日期后保存 Then 卡片下展示该规则且添加入口消失  
- Given 已有额外时间 When 查看卡片 Then 不再显示「+ 添加额外时间」，仅可编辑/删除  
- Given 表单日期重复 When 点保存 Then 提示生效日期不可重复  
- Given 同一营业时间已有额外时间 When 从 583 新建并勾选该营业时间 Then 提示已存在额外时间，请直接编辑  
