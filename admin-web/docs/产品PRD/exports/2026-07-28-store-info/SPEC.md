...
| dates | string[] | 是 | — | 多个单日生效时间（YYYY-MM-DD，去重升序，1～366） |
| fromDay / toDay / activeDays | 同 schedule | | | |
| mode | ExceptionMode | 是 | include | 生效/不生效 |
| scheduleIds | string[] | 是（保存时长度 1） | [] | 关联营业时间规则 id；空=待补全孤儿 |

存储键：`418-business-hour-exceptions`  

行为：独立设置项 **seq 583「额外时间」**（与 418 同组同级）提供总列表 CRUD；418 营业时间卡片下每条营业时间最多 1 条额外时间；一条规则可挂多个单日 `dates`；新建多选营业时间时按 id 拆成多条落盘；编辑已关联规则锁定营业时间，孤儿可重新单选；include=窗口内覆盖所选规则开闭市；exclude=窗口内暂停所选规则；删除 schedule 时从 scheduleIds 摘引用并立即落盘；旧 `fromDate/toDate` 与多关联记录在读取路径稳定拆分/合并（只读不写盘，下次保存持久化）；孤儿经 583 顶栏待补全列表编辑/删除。

兼容：读取时若无 `dates`，将旧 `fromDate~toDate`（或 legacy `date`）按天展开（自起点最多 366 天）；同一营业时间下多条规则合并日期并取最早基准字段。
...
| 418 列表 | 日程卡片下嵌套额外时间（已有则隐藏添加）；孤儿顶栏待补全 | 有额外时间时隐藏 + 添加 | 虚线空态卡片 | 新建营业时间 / 卡片内 + 添加额外时间 | SI-11～14 |
| 418 对话框 | 名称、日期、星期芯片、开闭市 | 校验失败不关窗 | — | 错误：请填写名称等 | SI-12 |
| 额外时间对话框 | 规则类型、多日期列表、作用于营业时间；MVP 卡片入口简化 | 校验失败不关窗；已关联编辑锁定营业时间 | — | 日期不可重复 / 已存在额外时间 | SI-14 |
