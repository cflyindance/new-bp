...
export function getBsTime(hours) {
  return hours.map((each) => {
    ...
    if (each.fromDayOfWeek && each.toDayOfWeek) {
      const fromBsIndex = WEEK_DAY.findIndex(
        (day) => day === each.fromDayOfWeek
      )
      ...
      } else if (toBsIndex < fromBsIndex) {
        each.bsDay = WEEK_DAY.slice(fromBsIndex).concat(
          WEEK_DAY.slice(0, toBsIndex + 1)
        )
      } else {
        each.bsDay = [WEEK_DAY[fromBsIndex]]
      }
    }
    return each
  })
