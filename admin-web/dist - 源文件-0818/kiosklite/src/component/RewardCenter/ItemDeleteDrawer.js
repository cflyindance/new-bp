...
import styles from './ItemDeleteDrawer.module.scss';
...
class ItemDeleteDrawer extends React.Component {
  constructor() {
    super();
    this.state = {
      isShowMore: false,
  ...
  handleConfirm = () => {
    ...
    this.props.handleConfirm(id, cloneItemList);
  ...
  componentDidMount() {
    // 深克隆，+-操作不影响redux中的数据
    const cloneArr = cloneDeep(this.props.itemList);
    // id过滤出不同属性，对应的菜品
  ...
  componentDidUpdate() {
    ...
    if (!this.state.cloneItemList.length) {
      // 真正清空菜
      this.props.handleConfirm(this.props.itemInfo.id, []);
      this.props.handleCloseDeleteModal();
      return;
    }
...
export default connect(
  mapStateToProps,
  {}
)(withTranslation()(ItemDeleteDrawer));
