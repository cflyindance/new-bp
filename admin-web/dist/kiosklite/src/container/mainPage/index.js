...
class MainPage extends React.Component {
  ...
  // 解析 POS ListSystemConfigurations 响应（与 deviceSetting / transferPosSetting 一致）
  parsePosSystemConfigXml = (data) => {
    try {
      const start = data?.indexOf('<soap:Body>');
      const end = data?.indexOf('</soap:Body>');
      if (start === -1 || end === -1) return [];
      const body = data?.substring(start + 11, end);
      const objTree = new XMLObjTree();
      const instanceList = objTree.parseXML(body);
      return (
        instanceList?.listsystemconfigurationsresponsetype
          ?.systemconfiguration || []
      );
    } catch (e) {
      posFrontLog(`parsePosSystemConfigXml${e}`);
      return [];
    }
  };
  ...
  /**
   * DP 开启时：POS 上 KIOSK_PAYMENT_TYPE 须同时包含卡(0)与现金(1)，与 margin 设备配置一致
   */
  syncDualPricePaymentTypesToPos = () => {
    if (!this.isDualPriceEnabled()) return;
    const userId = this.props.sysCookie?.kioskConfigUserId;
    if (!userId) return;

    const pt = this.props.systemConfig?.KIOSK_PAYMENT_TYPE?.value;
    if (pt && String(pt).includes('0') && String(pt).includes('1')) {
      return;
    }
    ...
    getKioskConfigFromPos()
      .then((res) => {
        const raw = res?.data;
        const rawR = this.parsePosSystemConfigXml(raw);
        const r = Array.isArray(rawR) ? rawR : rawR ? [rawR] : [];
        const list = [];
        if (r?.length) {
          posKeys.forEach((k, i) => {
            const result = r?.find((item) => item.name === k);
            if (!result) return;
            if (i === 1) {
              list.push({
                'app:id': result.id,
              ...
              if (k === 'KIOSK_PAYMENT_TYPE') {
                newArr = Array.from(new Set([...newArr, '0', '1']));
        ...
        // 与 transferPosSetting 一致，须包含三项后再写回 POS，避免部分覆盖
        if (list.length !== posKeys.length) return;
        ...
        return saveKioskConfigFromPos(str, userId);
      })
      .then((saveRes) => {
        if (!saveRes?.data) return;
        const body = saveRes.data;
        const s = body?.indexOf('<soap:Body>');
        const e = body?.indexOf('</soap:Body>');
        if (s === -1 || e === -1) return;
        const soapBody = body.substring(s + 11, e);
        const objTree = new XMLObjTree();
        const instanceList = objTree.parseXML(soapBody);
        const ok =
          instanceList?.updatesystemconfigurationresponsetype?.result
            ?.successful === 'true';
        if (ok) {
          fetchSystemConfig().then((fc) => {
            if (fc?.data) this.props.initConfigParams(fc.data);
          });
        }
      })
      .catch((err) => {
        console.error('syncDualPricePaymentTypesToPos', err);
      });
  };
  ...
  updateTipsConfig = (arr) => {
    ...
    // 设置DP后,小费模式未授权状态下,统一为先支付后小费
    if (tipConfig?.value !== 1 && !tipConfig.Authorization) {
      hasUpdated = true;
      configList = configList.map((item) =>
        item.id === 24 ? { ...item, value: 1 } : item
      );
    }
      ...
      Promise.race([
        ...
        .then(async (res) => {
          if (!res[1].data.result.successful) {
            ...
            this.syncDualPricePaymentTypesToPos();
            // 更新封面图
