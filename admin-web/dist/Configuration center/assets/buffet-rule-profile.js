(function () {
  ...
  var LEGACY_CAPABILITIES = {
    ...
    "KPOS-R08": { id: "KPOS-R08", label: "每轮指定菜品集最多菜品种（SPU）数", coverageStatus: "complete", level: "rule" },
    "KPOS-R09": { id: "KPOS-R09", label: "每人每轮指定菜品集最多菜品种（SPU）数", coverageStatus: "complete", level: "rule" },
    "KPOS-R10": { id: "KPOS-R10", label: "菜品集按份时限制相同菜品每轮最大份数", coverageStatus: "complete", level: "rule" },
    "KPOS-R11": { id: "KPOS-R11", label: "菜品集按种（SPU）时限制每种（SPU）菜品最大份数", coverageStatus: "complete", level: "rule" },
  ...
  function createDefaultScenarioRule(scenario, id) {
    ...
    return {
      ...
      method: defaultScenario.targetType === "dish_set" ? "按菜品集限购" : "按每种（SPU）菜品限购",
