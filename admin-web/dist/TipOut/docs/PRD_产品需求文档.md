...
| 产品模块 | 团队管理—小费管理 |
| 产品名称 | TipOut 小费分配系统 |
| 文档状态 | 保留 V2.5 功能基线；第 15 章为 V2.6 评审建议；第 16 章为 V2.7 最终范围，冲突建议按第 16 章执行 |
...
- 员工列表字段依次为：员工、角色、打卡工时、分配工时、分配前小费、扣除、分配获得、分配后小费、分配状态、明细入口；不展示班次。
...
顶部汇总字段：打卡工时、分配工时、手动工时记录、分配前、扣除、分配获得、实际获得。逐日字段：日期、考勤、打卡工时、分配工时、分配前、扣除、分配获得、实际获得、分配状态；不展示班次、上班时间和下班时间。
...
保留分配链路：`分配后小费 = 分配前小费 − 贡献入池金额 + 分配获得`。其中贡献入池金额按非负数参与公式，界面可展示负号；避免把已带负号的值再次相减。“扣除”建议改为“贡献入池”或“转入小费池”，说明这是员工之间的分配，不是雇主扣款。

另建支付链路：分配后应得、员工已持有现金、此前已发放、剩余应发。例：原始小费 $100，其中员工已持现金 $40、门店持有卡小费 $60；贡献 $20、分配获得 $10，则应得 $90，若无其他支付，剩余应发 $50。现金交回、历史补发、退款和负余额需要独立账项，不能只凭此简化公式覆盖所有场景。
