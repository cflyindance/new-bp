...
| 产品模块 | 团队管理—小费管理 |
| 产品名称 | TipOut 小费分配系统 |
| 文档状态 | 保留历史基线与版本记录；第 18 章为 V2.9 快照与重新确认规则；第 19 章为 V2.10 已在本地实现的考勤标签与打卡参与规则。历史不打卡规则的批量迁移仍待确认；生产发布另行确认 |
...
| 已有分配快照，未修改且未发放 | 展示最近一次已保存快照；再次进入或单纯查看不触发重新计算、覆盖或生成新版本 | “重新确认分配”不可用；保留已分配状态 |
| 已有快照，修改员工、工时、比例等分配明细数据 | 修改作为未确认草稿，可预览计算，但不得自动覆盖原快照 | 显示“存在未确认修改”；启用“重新确认分配”，原已分配状态不回退为未分配 |
| 点击“重新确认分配” | 校验当前修改，重新计算并成功保存新快照后替换原有效结果 | 清除未确认修改提示，按钮恢复不可用，日期、员工及角色汇总同步新快照 |
...
- 存在未确认修改时，阻止“确认已发放”，提示先重新确认分配，避免按旧快照发放。
...
| 无打卡记录 | 未打卡 | 灰色 | 保留员工行；分配工时为 0，从池分得为 0，不阻止其他打卡员工分配 |
...
- 考勤数据读取失败显示“考勤未知”，不能冒充“未打卡”，新确认保存必须拒绝未知考勤。新快照保存考勤标签依据；历史快照没有该字段时显示“考勤未知”，不使用当前考勤补写历史。已发放快照不重新计算或覆盖。

### 19.2 规则设置与自动分配边界

- 用户明确的新要求：小费规则仅支持打卡设置，不再支持创建“不打卡”规则。
- 符合分配条件不等于要求每位员工都已打卡：有有效打卡数据的员工正常参与，未打卡员工从池分得为 0；在营业、规则及数据校验通过后，沿用 V2.9 首次自动计算并保存快照的流程。
...
本地明细已接入标签及打卡规则的未打卡归零处理；规则编辑器隐藏并禁用“不打卡”选项，保存时校验打卡设置，自定义小费池也不再强制切换为不打卡。历史规则未被批量改写，用户编辑历史规则时须明确选择打卡后才可保存；已保存分配结果不会因本次升级自动重算。
...
八种基础组合为“有/无打卡 × 有/无订单 × 有/无小费”。无订单有小费的来源为直接上报小费，不伪造订单；订单销售额和订单小费可按明细求和。有效打卡员工仍由既有规则决定参与资格，未打卡员工从池分得为 0，不能因有订单而代替打卡。
