...
员工核对使用两个独立工时口径：打卡工时是考勤原值，分配工时是规则实际采用值。有效值最多保留两位小数并带 `h`；合法零值显示 `0 h`，缺失或非法值显示 `—`。分配工时必须按“小费池名称 · 规则名称”分行展示，不得将同池不同规则或跨池规则的工时相加。
...
- 打卡工时累计当前筛选范围内有效原始工时。部分日期缺失时展示 `N h（X/Y 天有记录）`，全部缺失显示 `—`；排序入口仅位于打卡工时列，缺失始终排末尾。
- 分配工时按 `poolId + ruleId` 分别累计并分行展示，每条规则独立计算 X/Y；非工时规则显示 `—`，不同规则不得求和，且不提供排序。
...
分配工时与打卡工时按规范化到两位小数后的业务值比较。不一致时只用颜色高亮对应规则的分配工时行，不展示差值、徽标或错误提示；无打卡但有合法手工工时时同样高亮。分配工时缺失、双方均缺失或非工时规则不高亮。
...
员工分配汇总与员工分配明细的 CSV、PDF、邮件导出均删除班次、上班时间、下班时间及旧单一“工时”字段，按顺序输出“打卡工时”“分配工时”。多规则分配工时在同一单元格内分行，并保留“小费池名称 · 规则名称”；员工汇总导出保留 X/Y 完整性文案，逐日导出只展示当天值。既有工时来源、考勤备注和手动工时明细继续保留。
