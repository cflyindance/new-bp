...
const listViewSource = fs.readFileSync(path.join(root, "dist/Configuration center/assets/buffet-rule-list-view.js"), "utf8");
...
const records = templates.map((template, index) => ({
  ...
  status: "disabled",
  authoringConfig: { name: `默认规则${index + 1}`, subject: template.key.startsWith("party_size") || template.key.endsWith("party_size") ? "party_size" : "order", targetType: template.key.includes("|dish_set") ? "dish_set" : "dish", enabledPeriods: [template.group === "order_lifetime" ? "order_lifetime" : "per_round"], deployStoreIds: [] },
}));
records.splice(2, 0, {
  id: "custom-1",
  status: "disabled",
  authoringConfig: { name: "自定义规则", subject: "order", targetType: "category", enabledPeriods: [], deployStoreIds: [] },
});

const list = { innerHTML: "", onclick: null, addEventListener() {} };
const createRule = { onclick: null, addEventListener() {} };
const passiveElement = { value: "", checked: false, hidden: false, textContent: "", addEventListener() {}, setAttribute() {} };
const context = {
  window: {
  ...
  document: { getElementById: id => id === "ruleList" ? list : id === "createRule" ? createRule : passiveElement, addEventListener() {} },
  location: { href: "" },
...
vm.runInNewContext(listViewSource, context, { filename: "buffet-rule-list-view.js" });
...
assert.equal(rendered.includes("本调研不适用"), false, "列表不得显示已废弃的调研占位文案");

const api = context.window.__BUFFET_RULE_LIST_TEST_API__;
assert.ok(api, "列表页应暴露轻量测试接口");
assert.equal(typeof api.reload, "function");
