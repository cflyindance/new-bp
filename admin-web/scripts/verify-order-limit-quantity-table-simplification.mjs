...
const rowsRenderer = source.match(/function renderMergedLimitRows\(draft[\s\S]*?(?=\n\s*function renderMergedSceneTable)/)?.[0];
...
assert.match(source, /function renderMergedSceneTable[\s\S]*?renderMergedLimitRows/, "批量选择列和数量行应继续组合渲染");
...
assert.match(source, /var applyValue\s*=\s*Number\(applyInput\.value\)/, "批量应用应仅从数量输入框读取值");
assert.match(source, /configured:\s*true,\s*value:\s*applyValue/, "批量显式输入 0 应继续保存为已配置状态");
