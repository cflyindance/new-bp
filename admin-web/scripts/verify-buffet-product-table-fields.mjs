...
assert.match(flow, /syncStoreTargetsFromStructure[\s\S]*?dishSetMembers = items\.map[\s\S]*?name: meta && meta\.name \|\| item\.name[\s\S]*?categoryName: meta && meta\.categoryName/);
assert.match(flow, /buffetSceneRows[\s\S]*?productName: meta\.productName/);
assert.match(flow, /renderCrossStoreSceneRow[\s\S]*?buffetDisplayName\(target, lineName, row\.productName\)/);
