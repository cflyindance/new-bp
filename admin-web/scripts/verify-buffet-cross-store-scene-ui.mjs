...
assert.match(flow, /var quotaColumns = draft\.targetType === "dish_set"/);
assert.match(flow, /var columnCount = \(draft\.targetType === "category" \? 6 : 7\) \+ quotaColumns\.length/);
assert.match(flow, /var quotaHeadings = quotaColumns\.map/);
assert.doesNotMatch(flow, /identityHeadings \+ '<th>限购数量<\/th>'/);
