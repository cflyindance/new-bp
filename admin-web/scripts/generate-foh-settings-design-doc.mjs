...
push(
  ...
  "### 3.11 下单与规则边界收紧（已确认）",
  "",
  "seq **600**（全局纯展示模式，不可加购）自下单规则迁入食客端菜单（v1.3 为 **`guest-menu-global`**）；v1.10/v1.11 送厨迁入；**v1.12** 并入火锅/品类 **571–575**。按时计价规则现作为组级独立管理区，不占功能 seq。",
  "",
  "| 配置 | seq | 说明 |",
  "|------|-----|------|",
...
for (let i = 0; i < order.length; i++) {
  ...
  if (k === "foh-guest-duration-scenarios") {
    push(
      "",
      "**组内展示顺序**：独立按时计价规则区 → 571 品类先下单 → 674 用餐时长限制 → 577 展示用餐时长 → 578 倒计时 → 579 剩余提示 → 580 提示后禁下单。",
      "",
  ...
  if (k === "guest-order-rules") {
    push(
      "",
      "**组内展示顺序**：502/581 送厨 → 571/572/574/575/573 火锅·品类 → 577–580 用餐时长 → 597/598 轮次规则。按时计价规则在计时与自助餐规则组内独立展示。",
      "",
