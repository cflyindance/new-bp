...
const reasons = {
  ...
  "brand-menu-presentation":
    "本店品牌启用、品牌↔菜单绑定及多品牌首页/点餐展示（547 数据、530 品牌页作为首页含产线多选；531 已并入 530；548 已并入 547）；对标连锁「Location 级品牌配置」。",
...
const MERGED_EXCLUDED_SEQS = new Set([548, 531]);
...
const SCENE_SUMMARY_OVERRIDE_BY_SEQ = new Map([
  ...
  [420, "批量删除地址数据（运维；同上）"],
  [530, "品牌页作为首页：开启=所选产线（Kiosk/eMenu/SDI）以品牌卡片为首页并先选品牌点餐；关闭=常规封面图首页（见营销中心广告）"],
...
push(
  "# 门店管理 · 设置二级导航重设计方案",
  "",
  "> 文档版本：v1.5（531 品牌页作为首页自主页迁入品牌与菜单）  ",
  `> 数据范围：门店 hub catalog **${rows.length}** 条（设置 8 + 品牌与菜单 3；不含 seq 421；548 并入 547）  `,
  ...
  "### 1.2 设计目标",
  "",
  "- 滑层「设置」**3 组 8 条**；**品牌与菜单**独立页 **3 条**（547 → 531 → 530）",
...
push(
  ...
  "| **v1.5** | **531 迁出主页** | 品牌页作为首页并入 `/stores/brand-menu`；主页移除「设置」入口 |",
  "| **v1.6** | **531 并入 530** | 品牌页作为首页与品牌设置合并为单开关 + Kiosk/eMenu/SDI 产线多选 |",
  "",
  "### 3.2 v1.3 调整明细",
  "",
  "| 调整 | seq | 原组 | 新组 | 理由 |",
  ...
  "### 3.3 组内边界说明",
  "",
  "- **547 → 530**：547 配置品牌数据；530「品牌页作为首页」控制所选产线是否以品牌卡片为首页并先选品牌（原 seq 531 已并入）。",
  "- **530 封面图**：关闭时常规首页封面见营销中心 · 广告（seq 555）。",
...
push(
  ...
  "| 品牌与菜单 · **547** | 本店品牌列表；新增/编辑（名称、图片、418 营业时间多选、品牌菜单多选） | `module-settings-store-brand-management-ui.ts` |",
  "| 品牌与菜单 · **530** | 展示开关（先选品牌再点餐） | — |",
  "",
