/**
 * 将促销中心 2 组分类写入 docs/项目文档/配置归类-分组映射.csv
 * 运行：node scripts/apply-promotion-settings-mapping.mjs
 */
...
for (const [key, seqs] of Object.entries(assignMap)) {
  for (const seq of seqs) {
    promoAssign.set(seq, { groupTitle: titles[key], groupKey: key });
  }
}
...
for (const line of lines) {
  ...
  const next = promoAssign.get(seq);
  if (next) {
...
if (updated !== promoAssign.size) {
  throw new Error(`预期更新 ${promoAssign.size} 条，实际 ${updated} 条`);
}
